...
def monin_obukhov_stability(z0: float, z: float, temp_c: float, temp_surf: float, 
    ...
    for _ in range(5):
        ...
        # 
        # Rango físico: ζ ∈ (-∞, +∞) teóricamente, pero:
        # - ζ < -2: Convección libre extrema (térmicas fuertes)
        # - -2 < ζ < 0: Inestable (mixing turbulento)
        # - ζ = 0: Neutral (mecánico puro)
        # - 0 < ζ < 1: Estable débil
        # - ζ > 1: Muy estable (inversión térmica, turbulencia suprimida)
        # 
        # CLAMP ζ = ±9.0:
