...
class PanelViewModel:
    ...
    def obtener_panel_superior(self, lat: float, lon: float, estacion: str) -> Dict[str, Any]:
        ...
        def format_coord(val):
            # Si es float, mostrar todos los decimales significativos
            if isinstance(val, float):
                # Elimina ceros a la derecha pero nunca trunca precisión
                return ('%.10f' % val).rstrip('0').rstrip('.') if '.' in ('%.10f' % val) else str(val)
            return str(val)
        return {
