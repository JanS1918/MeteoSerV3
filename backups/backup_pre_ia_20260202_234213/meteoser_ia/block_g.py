...
class ClusterRegistry:
    ...
    def register_node(self, host: str = "127.0.0.1", port: int = 0, metadata: Dict[str, Any] = None) -> NodeInfo:
        with self._lock:
            nid = f"node_{uuid.uuid4().hex[:8]}"
            node = NodeInfo(node_id=nid, host=host, port=port or 0, metadata=metadata or {})
    ...
    def snapshot(self) -> ClusterStateSnapshot:
        with self._lock:
            leader = next((n.node_id for n in self._nodes.values() if n.is_leader), None)
            return ClusterStateSnapshot(nodes=dict(self._nodes), leader_id=leader, last_updated=time.time())
...
def _prune_dead_nodes() -> None:
    ...
    for nid in removed:
        ...
        inc = block_d.INCIDENT_LOG.create(module="cluster", severity="critical", description=f"Node {nid} timed out")
...
def _gather_critical_state() -> Dict[str, Any]:
    ...
    try:
        ...
        state["alg_versions"] = []
    try:
        state["health"] = {s.module: s.healthy for s in block_d.HEALTH_REGISTRY.list_all()}
    except Exception:
        state["health"] = {}
...
def perform_failover(failed_node_id: str) -> None:
    ...
    try:
        if hasattr(block_d, "_invoke_ha_hook"):
            inc = block_d.INCIDENT_LOG.create(module="cluster", severity="critical", description=f"Failover triggered for {failed_node_id}")
...
class LocalNodeAgent(threading.Thread):
    def __init__(self, host: str = "127.0.0.1", port: int = 0, metadata: Dict[str, Any] = None) -> None:
    ...
    def run(self) -> None:
        self.node = CLUSTER_REGISTRY.register_node(host=self.host, port=self.port, metadata=self.metadata)
        try:
...
def start_local_agent(host: str = "127.0.0.1", port: int = 0, metadata: Dict[str, Any] = None) -> LocalNodeAgent:
...
def list_cluster_nodes() -> Dict[str, Any]:
    snap = CLUSTER_REGISTRY.snapshot()
    nodes = {nid: {"host": n.host, "port": n.port, "is_leader": n.is_leader, "last_heartbeat": n.last_heartbeat} for nid, n in snap.nodes.items()}
