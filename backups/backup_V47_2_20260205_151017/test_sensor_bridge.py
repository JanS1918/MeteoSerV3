...
def test_sensor_bridge():
    ...
    print(f"✅ {rellenados} parámetros rellenados")
    
    if rellenados == 0:
        print("❌ ERROR: No se cargaron parámetros. ¿last_sensores.json existe?")
        return False
    
    # Test 2: Verificar estructura
    print("\n[TEST 2] Verificar estructura del historial...")
    if not system.historial_sensores:
        print("❌ ERROR: historial_sensores está vacío")
        return False
    
    print(f"✅ Parámetros en historial: {len(system.historial_sensores)}")
    ...
    if historico_path.exists():
        ...
        print(f"✅ Archivo creado: {historico_path}")
        ...
        print("⚠️ sensores_historico.json no existe aún")
    ...
    print(f"✅ Segunda carga en {tiempo:.2f} ms ({rellenados2} parámetros)")
