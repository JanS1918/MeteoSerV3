...
class BusCapasInformacion:
    ...
    def consumir_elite(self, parametro_abstracto: str, consumidor: str) -> tuple:
        ...
        if parametro_normalizado != parametro_abstracto:
            logger.warning(
                "⚠️ Parámetro no canónico recibido: '%s' -> '%s'",
                parametro_abstracto,
        ...
        if not jerarquia:
            logger.error(
                f"❌ Parámetro '{parametro_normalizado}' NO está en Registro de Élite V30.1"
            )
            return (None, 0, None)
        ...
        logger.error(
            f"❌ {consumidor} pidió '{parametro_normalizado}' - "
