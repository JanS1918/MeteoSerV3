"""
═══════════════════════════════════════════════════════════════════════════
🛡️ V47.0 MOS (MODEL OUTPUT STATISTICS) - VALIDACIÓN INTERNA
═══════════════════════════════════════════════════════════════════════════
...
class MOSValidator:
    ...
    def __init__(self, bus=None):
        ...
        logging.info("🛡️ MOS Validator V47.0 inicializado")
    ...
    def validar_predicciones_pendientes(self) -> Dict:
        ...
        for prediccion in self.predicciones:
            ...
            logging.info(f"✅ MOS Validación: {parametro} - Predicho={valor_predicho:.2f}, Real={valor_real:.2f}, Error={error:+.2f}")
        ...
        for parametro, vals in por_parametro.items():
            ...
            if abs(bias) > umbral:
                ...
                if coef_key in self.resultados["coeficientes"]:
                    ...
                    logging.warning(
                        f"⚠️ MOS AJUSTE: {parametro} - BIAS={bias:+.2f}, "
...
if __name__ == "__main__":
    ...
    print(f"\n📊 Estadísticas T_min: {stats_tmin}")
    
    confianza_global = validator.obtener_confianza_global()
    print(f"\n🛡️ Confianza global del sistema: {confianza_global}%")
    
    print("\n✅ MOS Validator V47.0 - OPERATIVO")
