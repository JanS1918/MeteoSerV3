...
class BiasSensorBusPublisher:
    ...
    def __init__(self, mqtt_client=None):
        ...
        logger.info("✅ BiasSensorBusPublisher inicializado")
    ...
    def publish_to_bus(self, sensor_name: str) -> bool:
        ...
        if sensor_name not in self.bias_registry:
            logger.warning(f"⚠️ {sensor_name} no tiene bias registrado")
            return False
        ...
        if self.mqtt_client:
            try:
                ...
                logger.info(f"✅ Publicado bias {sensor_name} en {topic}")
                return True
            except Exception as e:
                logger.error(f"❌ Error publicando bias {sensor_name}: {e}")
                return False
        else:
