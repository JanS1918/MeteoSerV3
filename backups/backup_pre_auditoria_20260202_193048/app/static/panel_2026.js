...
function renderizarArcoSolarHUD(datosSol) {
    ...
    // ⚡ ESTÉTICA CRONÓGRAFO SUIZO: Líneas ultra-finas, color dorado solar
    const colorLinea = '#ffd700';  // Dorado solar
...
function renderizarArcoLunarHUD(datosLuna) {
    ...
    // ⚡ ESTÉTICA CRONÓGRAFO SUIZO: Plateado nocturno
    const colorLinea = '#4fc3f7';  // Cian/azul lunar
...
function actualizarMetricasHUD(data) {
    ...
    if (data.sol.amanecer && data.sol.anochecer) {
        // Parsear horas amanecer/anochecer (formato "HH:MM")
        const [amH, amM] = data.sol.amanecer.split(':').map(Number);
        const [anH, anM] = data.sol.anochecer.split(':').map(Number);
        
        // Convertir a minutos totales
        const minAmanecer = amH * 60 + amM;
        const minAnochecer = anH * 60 + anM;
        
        // Duración del día en minutos
        const duracionDiaMin = minAnochecer - minAmanecer;
        const hDia = Math.floor(duracionDiaMin / 60);
        const mDia = duracionDiaMin % 60;
        
        // Duración de la noche en minutos (24h - día)
        const duracionNocheMin = 24 * 60 - duracionDiaMin;
        const hNoche = Math.floor(duracionNocheMin / 60);
        const mNoche = duracionNocheMin % 60;
        
        // Mostrar duración del día
        const ojoDuracionDiaElem = document.getElementById('ojo-duracion-dia');
        if (ojoDuracionDiaElem) {
            ojoDuracionDiaElem.textContent = `${hDia}h ${mDia}m`;
        }
        
        // Mostrar duración de la noche
        const ojoDuracionNocheElem = document.getElementById('ojo-duracion-noche');
        if (ojoDuracionNocheElem) {
            ojoDuracionNocheElem.textContent = `${hNoche}h ${mNoche}m`;
        }
    }
...
function configurarEventos() {
    ...
    if (btnNoticias) {
        btnNoticias.addEventListener('click', async () => {
            try {
                const resp = await fetch(`${UI_API_BASE}/asistente/noticias`);
                const data = await resp.json();
                const noticias = data.noticias || {};
                const categorias = data.categorias || [];
                const items = Object.values(noticias).flat();
                const lista = items.length
                    ? `<ul>${items.map(n => `<li><strong>${n.titulo || n.title || 'Noticia'}</strong><br>${n.descripcion || n.summary || ''}</li>`).join('')}</ul>`
                    : '<p>No hay noticias disponibles.</p>';
                mostrarOverlay('Noticias', `<p>Categorías: ${categorias.join(', ')}</p>${lista}`);
            } catch (err) {
                console.error('Error cargando noticias:', err);
                mostrarOverlay('Noticias', '<p>Error cargando noticias.</p>');
            }
        });
    }
    ...
    if (btnAsistente) {
        btnAsistente.addEventListener('click', async () => {
            try {
                const resp = await fetch(`${UI_API_BASE}/asistente/estado`);
                const data = await resp.json();
                const resumen = `
                    <p><strong>Eventos:</strong> ${(data.eventos || []).length}</p>
                    <p><strong>Alarmas:</strong> ${(data.alarmas || []).length}</p>
                    <p><strong>Tareas:</strong> ${(data.tareas || []).length}</p>
                    <p><strong>Lista compra:</strong> ${(data.lista_compra || []).length}</p>
                    <p><strong>Recomendaciones:</strong> ${(data.recomendaciones || []).length}</p>
                `;
                mostrarOverlay('Asistente', resumen);
            } catch (err) {
                console.error('Error cargando asistente:', err);
                mostrarOverlay('Asistente', '<p>Error cargando estado del asistente.</p>');
            }
        });
    }
    ...
    if (btnConfiguracion) {
        btnConfiguracion.addEventListener('click', async () => {
            try {
                const [layoutResp, panelsResp] = await Promise.all([
                    fetch(`${UI_API_BASE}/config/layout`),
                    fetch(`${UI_API_BASE}/config/panels`)
                ]);
                const layout = await layoutResp.json();
                const panels = await panelsResp.json();
                const html = `
                    <h3>Layout</h3>
                ...
                `;
                mostrarOverlay('Configuración', html);
            } catch (err) {
                console.error('Error cargando configuración:', err);
                mostrarOverlay('Configuración', '<p>Error cargando configuración.</p>');
            }
        });
    }
    ...
    if (btnFeedback) {
        btnFeedback.addEventListener('click', () => {
            const html = `
                <p>Registra feedback de una predicción o índice.</p>
                <label>Tipo <input id="fb-tipo" placeholder="prediccion" /></label><br>
                <label>Nombre <input id="fb-nombre" placeholder="utci" /></label><br>
                <label>Valor predicho <input id="fb-valor" type="number" /></label><br>
                <label>Valor real <input id="fb-valor-real" type="number" /></label><br>
                <label>Feedback <select id="fb-feedback"><option value="acierto">acierto</option><option value="error">error</option></select></label><br>
                <button id="fb-enviar" class="btn-control">Enviar</button>
            `;
            mostrarOverlay('Feedback', html);
            const btnEnviar = document.getElementById('fb-enviar');
            if (btnEnviar) {
                btnEnviar.addEventListener('click', async () => {
                    const payload = {
                        tipo: document.getElementById('fb-tipo')?.value || 'prediccion',
                        nombre: document.getElementById('fb-nombre')?.value || null,
                        valor: parseFloat(document.getElementById('fb-valor')?.value),
                        valor_real: parseFloat(document.getElementById('fb-valor-real')?.value),
                        feedback: document.getElementById('fb-feedback')?.value || 'acierto'
                    };
                    try {
                        await fetch(`${UI_API_BASE}/feedback_prediccion`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify(payload)
                        });
                        mostrarOverlay('Feedback', '<p>Feedback enviado.</p>');
                    } catch (err) {
                        console.error('Error enviando feedback:', err);
                        mostrarOverlay('Feedback', '<p>Error enviando feedback.</p>');
                    }
                });
            }
        });
...
function mostrarOverlay(titulo, html) {
    const overlay = document.getElementById('submenu-overlay');
    const contenido = document.getElementById('submenu-contenido');
    if (!overlay || !contenido) return;
    contenido.innerHTML = `<h2>${titulo}</h2>${html || ''}`;
    overlay.style.display = 'flex';
    cajonBloqueadoId = null;
}
