...
class BLEDriver:
        ...
        try:
            ...
            for device in discovered:
                device_info = BLEDeviceInfo(
                    ...
                    service_uuids=[str(uuid) for uuid in device.metadata.get('uuids', [])],
                ...
                logger.info(
                    f"📡 BLE: {device.name or 'Unknown'} | "
                    f"{device.address} | RSSI={device.rssi}dBm | "
                    f"{sensor_type}"
                )
    ...
    async def connect(self, address: str, timeout: float = 10.0) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ Conectado a BLE {address}")
            return True
