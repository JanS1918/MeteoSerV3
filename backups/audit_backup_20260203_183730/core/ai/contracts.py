...
class ContractParser:
    ...
    def save_contract(self, contract: Dict, filename: Optional[str] = None) -> Optional[str]:
        ...
        if not contract_id:
            logger.error("❌ Contrato sin id, no se puede guardar")
            return None
        ...
        try:
            ...
            self.loaded_contracts[contract_id] = contract
            logger.info(f"💾 Contrato guardado: {file_path}")
            return str(file_path)
        except Exception as e:
            logger.error(f"❌ Error guardando contrato {contract_id}: {e}")
            return None
    ...
    def update_contract(self, contract_id: str, updates: Dict[str, Any]) -> Optional[Dict]:
        ...
        if not contract:
            logger.error(f"❌ Contrato no cargado: {contract_id}")
            return None
    ...
    def delete_contract(self, contract_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"❌ Error eliminando contrato {contract_id}: {e}")
            return False
    ...
    def generate_driver_scaffold(self, contract: Dict) -> str:
        ...
        lines.extend([
            ...
            '            logger.error(f"❌ Error HTTP leyendo {url}: {e}")',
            '            return None',
            '',
            ...
            '            logger.error(f"❌ Error HTTP escribiendo {url}: {e}")',
            '            return False',
            '',
            ...
            '            if not path:',
            '                logger.error("❌ Protocolo file requiere path o data_file")',
            '                return None',
            '            return self._read_from_file(path)',
            '        if proto_type in ("http", "https"):',
            '            url = proto.get("url") or self.config.get("url")',
            '            if not url:',
            '                logger.error("❌ Protocolo HTTP requiere url")',
            '                return None',
            '            return self._read_from_http(url)',
            '        logger.error(f"❌ Protocolo no soportado para lectura: {proto_type}")',
            '        return None',
            '',
            ...
            '        logger.error(f"❌ Protocolo no soportado para escritura: {proto_type}")',
            '        return False',
        ])
