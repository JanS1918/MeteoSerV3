...
class HealthRegistry:
    ...
    def update(self, status: HealthStatus) -> None:
        ...
        logger.debug(f"HealthRegistry: actualizado estado de {status.module}: healthy={status.healthy}")
...
class IncidentLog:
    ...
    def create(self, module: str, severity: str, description: str) -> Incident:
        inc = Incident(id=f"inc_{int(time.time()*1000)}", timestamp=time.time(), module=module, severity=severity, description=description)
        self._incidents.append(inc)
        logger.warning(f"Nuevo incidente: {inc.id} module={module} severity={severity} desc={description}")
        return inc
...
def check_block_a() -> HealthStatus:
    try:
        ...
        return HealthStatus(module="block_a", healthy=healthy, last_checked=time.time(), details=details)
    except Exception as e:
        return HealthStatus(module="block_a", healthy=False, last_checked=time.time(), details={"error": str(e)})

def check_block_b() -> HealthStatus:
    try:
        block_b.PRESENCE_MODEL.update_from_sensors()
        return HealthStatus(module="block_b", healthy=True, last_checked=time.time(), details={})
    except Exception as e:
        return HealthStatus(module="block_b", healthy=False, last_checked=time.time(), details={"error": str(e)})

def check_block_c() -> HealthStatus:
    try:
        versions = block_c.ALGO_REPO.list_versions()
        return HealthStatus(module="block_c", healthy=True, last_checked=time.time(), details={"alg_versions": len(versions)})
    except Exception as e:
        return HealthStatus(module="block_c", healthy=False, last_checked=time.time(), details={"error": str(e)})
...
def autocure_incident(module_name: str, incident: Incident) -> bool:
    ...
    for attempt in range(1, AUTOCURE_RETRY_LIMIT + 1):
        try:
            ...
            logger.warning(f"Error en intento de reinicio {attempt} para {module_name}: {e}")
    _mark_for_rollback(module_name, f"Autocuración fallida tras {AUTOCURE_RETRY_LIMIT} intentos")
    INCIDENT_LOG.resolve(incident.id, f"Autocuración fallida; rollback lógico marcado")
    return False
            ...
            try:
                for module, check in HEALTH_CHECKS.items():
                    ...
                    if not status.healthy:
                        inc = INCIDENT_LOG.create(module=module, severity="critical", description=f"Health check failed: {status.details}")
                        autocure_incident(module, inc)
                    else:
                        last_ok_times[module] = time.time()
                time.sleep(self.interval)
            except Exception as e:
                logger.error(f"Error en loop del watchdog: {e}\n{traceback.format_exc()}")
                time.sleep(self.interval)
...
def get_system_health_report() -> Dict[str, Any]:
    statuses = {s.module: {"healthy": s.healthy, "last_checked": s.last_checked, "details": s.details} for s in HEALTH_REGISTRY.list_all()}
    incidents = [{"id": i.id, "module": i.module, "severity": i.severity, "desc": i.description, "resolved": i.resolved} for i in INCIDENT_LOG._incidents]
