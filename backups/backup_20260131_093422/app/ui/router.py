...
def generar_grupos_desde_sistema(contexto: Dict[str, Any]) -> List[GrupoValores]:
    ...
    uv = _extraer_valor_sensor(sensores, ['uv', 'indice_uv'])
    valor_uv, fiab_uv = _aplicar_fallback(uv.get('valor') if uv else None, 'elevacion_solar', 'uv')
    valores_rad.append(ValorSistema(
        nombre='Índice UV',
        tipo='sensor',
        valor=valor_uv,
...
templates_path = templates_path.resolve()  # Ruta absoluta
...
async def panel_principal(request: Request):
    """
    Renderiza el panel principal de la nueva UI.
    """
