...
def monin_obukhov_stability(z0: float, z: float, temp_c: float, temp_surf: float, 
                            viento_ms: float, rn: float, presion_hpa: float = None) -> Dict[str, float]:
    """
    Estabilidad atmosférica Monin-Obukhov con iteración Businger-Dyer.
    ARQUITECTURA DE RESILIENCIA: Con fallback automático y Zilitinkevich.
    ...
    # Aplicar fallback a todos los parámetros
    ...
    # SINTONIZACIÓN 2026: Gravedad dinámica (Somigliana)
    engine = PhysicsEngine2026(
        latitud=41.5513,  # Argentona, Barcelona
        temperatura_k=T_k,
        presion_pa=presion_val * 100.0
    )
    g, _ = engine.gravedad_somigliana()
    
    k = 0.4  # Constante von Kármán
    cp, _ = engine.calor_especifico_dinamico()
    rho = 1.225  # kg/m³ (podría dinamizarse con Tv)
    ...
    # RELACIÓN DE ZILITINKEVICH: Rugosidad térmica ≠ Rugosidad mecánica
    ...
    for _ in range(5):
        ...
        # Funciones psi Businger-Dyer
        zeta = z_val / L if L != float('inf') else 0
        # LEY DEL ENTERO 2026: Liberar zeta a ±9 para detectar inversiones térmicas reales
        zeta = max(-9.0, min(9.0, zeta))
    ...
    # Determinar status
    status = estado_rn if estado_rn != EstadoFisico.REAL else EstadoFisico.REAL
...
def nubosidad_romps_2017(temp_c: float, presion_kpa: float, humedad: float) -> Dict[str, float]:
    ...
    # Punto de rocío CON FACTOR DE MEJORA DE GREENSPAN
    ...
    # Estimar CAPE (Convective Available Potential Energy)
