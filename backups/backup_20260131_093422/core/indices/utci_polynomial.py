...
def utci_polynomial(ta: float, tmrt: float, va: float, vp: float,
    ...
    # Clipping de valores para evitar extrapolaciones peligrosas
    ta = max(-50, min(60, ta))
    va = max(0.1, min(17, va))
    vp = max(0, min(54, vp))
    
    # Calcular diferencia de temperatura radiante
    d_tr = tmrt - ta
    d_tr = max(-50, min(120, d_tr))
    ...
    # Factor de humedad (presión de vapor)
    if ta > -40:
        es = 6.105 * math.exp(17.27 * ta / (237.7 + ta))
...
def utci_iterative(ta: float, tmrt: float, va: float, rh: float,
    ...
    """
    # Convertir RH a presión de vapor
    if ta > -40:
        es = 6.105 * math.exp(17.27 * ta / (237.7 + ta))
        vp = (rh / 100.0) * es
    else:
