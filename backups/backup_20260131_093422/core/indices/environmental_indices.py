...
def to_physics_safe(valor: Any, tipo_indice: Optional[str] = None, path: str = "") -> Tuple[Any, Optional[str]]:
    ...
    if status == "SATURADO":
        logger.info(f"Saturación en '{path}' | tipo={tipo} | valor→{v}")
    return v, status
...
def indice_heat_index_c(temp_c: float, humedad: float, contexto) -> float:
    """
    Heat Index (Índice de Calor) según NOAA.
    Implementación basal para reemplazar referencia muerta.
...
# PURGA: Heat Index simple reemplazado por UTCI Fiala completo


# PURGA: Wind Chill simple reemplazado por UTCI Fiala


def indice_bulbo_humedo_c(temp_c: float, humedad: float, contexto) -> float:
        """DEPRECADO: usar indice_utci_completo."""
...
def indice_entalpia_kjkg(temp_c: float, humedad: float, presion_kpa: float = 101.325, lat: float = None, lon: float = None, alt: float = None, dt = None) -> float:
    # Entalpía del aire húmedo CON FACTOR DE GREENSPAN
    T_k = temp_c + 273.15
    p_pa = presion_kpa * 1000.0
    
    es_base: float = 0.6108 * math.exp((17.27 * temp_c) / (temp_c + 237.3))
    Bm = -1.6e-5 + 1.8e-8 * T_k
    f_greenspan = math.exp(Bm * p_pa / (8.314472 * T_k))
    es: float = es_base * f_greenspan
...
class EnvironmentalIndices:
    ...
    def _pm_humidity_correction(self, pm_val: float, rh: float, base_sensor: str | None = None) -> float:
        ...
        # Usar calibrador dinámico con Particle Filter
        try:
...
def saturacion_vapor_tetens_simple(temp_c: float, presion_pa: float = None) -> float:
    """
    BÁSICO: Tetens simplificado.
    Fallback más estable si falla todo lo demás.
    """
    import math
    if temp_c >= 0:
        return 610.78 * math.exp((17.27 * temp_c) / (temp_c + 237.3))
    else:
        return 610.78 * math.exp((21.875 * temp_c) / (temp_c + 265.5))
...
def calcular_saturacion_vapor_con_fallback(temp_c: float, presion_pa: float = None) -> tuple:
    ...
    funciones = [
        ...
        ('tetens_simple', saturacion_vapor_tetens_simple)
    ]
