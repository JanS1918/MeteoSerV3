...
def modelo_pennycuick_vuelo(
    ...
    # Densidad del aire (kg/m³) - USA PRESIÓN REAL DE ARGENTONA
    T_k = temperatura_c + 273.15
    rho = (presion_hpa * 100.0) / (287.05 * T_k)
    ...
    # Potencia requerida (modelo Pennycuick simplificado)
    # P = P_ind + P_par + P_pro
    # P_ind: potencia inducida (vórtices de punta de ala)
    # P_par: potencia parásita (arrastre del cuerpo)
    # P_pro: potencia de perfil (arrastre alar)
    
    # Potencia inducida
    k = 1.2  # Factor de eficiencia de Oswald
    P_ind = (k * W**2) / (0.5 * rho * S * V_opt * (math.pi * AR))
    
    # Potencia parásita (arrastre)
