...
class DialogManager:
    def __init__(self) -> None:
        self._intent_handlers: Dict[str, Callable[[ConversationContext, Dict[str, Any]], DialogResponse]] = {}
    ...
    def register_intent_handler(self, intent: str, handler: Callable[[ConversationContext, Dict[str, Any]], DialogResponse]) -> None:
    ...
    def handle_text(self, session_id: str, text: str) -> DialogResponse:
        ...
        logger.info(f"NLU detected intent={intent} conf={nlu['confidence']:.2f} for session={session_id}")
        resp = handler(ctx, nlu)
        ctx.history.append({"user": text, "nlu": nlu, "response": resp.text, "ts": time.time()})
        return resp

    def _handle_greeting(self, ctx: ConversationContext, nlu: Dict[str, Any]) -> DialogResponse:
        text = "Hola. ¿En qué puedo ayudarte hoy?"
        return DialogResponse(text=text)

    def _handle_ask_clothing(self, ctx: ConversationContext, nlu: Dict[str, Any]) -> DialogResponse:
    ...
    def _handle_ask_status(self, ctx: ConversationContext, nlu: Dict[str, Any]) -> DialogResponse:
        snapshot = block_a.SENSOR_REGISTRY.snapshot()
        sensors = snapshot.sensors
        if not sensors:
            return DialogResponse(text="No detecto sensores registrados en este momento.")
        summary_lines = []
        for sid in list(sensors.keys())[:3]:
            ...
            if r and r.valid:
                ...
                summary_lines.append(f"{sensors[sid].name}: lectura inválida o no disponible")
        text = "Resumen de sensores: " + " | ".join(summary_lines)
        return DialogResponse(text=text)

    def _handle_deploy_algorithm(self, ctx: ConversationContext, nlu: Dict[str, Any]) -> DialogResponse:
        ctx.slots["pending_deploy"] = True
        text = "He preparado el despliegue del algoritmo. Confirma con 'sí, desplegar' para continuar."
        return DialogResponse(text=text)

    def _handle_ack(self, ctx: ConversationContext, nlu: Dict[str, Any]) -> DialogResponse:
        if ctx.slots.get("pending_deploy"):
            versions = block_c.ALGO_REPO.list_versions()
            if not versions:
                return DialogResponse(text="No hay ninguna versión de algoritmo lista para desplegar.")
            latest = sorted(versions, key=lambda v: v.timestamp)[-1]
            ok = block_c.deploy_algorithm(latest.id)
            if ok:
                ctx.slots.pop("pending_deploy", None)
                return DialogResponse(text=f"Versión {latest.id} marcada como desplegada (acción lógica).")
            else:
                return DialogResponse(text="No se pudo marcar la versión como desplegada.")
        return DialogResponse(text="Perfecto.")

    def _handle_fallback(self, ctx: ConversationContext, nlu: Dict[str, Any]) -> DialogResponse:
        if EXTERNAL_INTEGRATION_MODE == block_a.ExternalIntegrationMode.MOCK:
            return DialogResponse(text="Lo siento, no he entendido. ¿Puedes reformularlo?")
        ...
        data = {
            ...
            "messages": [
                {"role": "system", "content": "Eres un asistente meteorológico integrado en MeteoSer. Responde en español y de forma clara."},
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Error LLM externo: {response.status_code} {response.text}")
                return DialogResponse(text="No se pudo obtener respuesta de la IA externa (OpenRouter). Intenta de nuevo más tarde.")
        except Exception as e:
            logger.error(f"Excepción al consultar LLM externo: {e}")
            return DialogResponse(text="Error al conectar con la IA externa. Intenta de nuevo más tarde.")
...
def handle_user_text(session_id: str, text: str) -> Dict[str, Any]:
    ...
    return {"text": resp.text, "audio": bool(audio), "end_session": resp.end_session, "metadata": resp.metadata}
