...
class RecuperadorDatosEcowitt:
    ...
    def obtener_datos_mqtt_local(self):
        ...
        """
        logger.info("🔍 Verificando MQTT broker local...")
        ...
        try:
            ...
            def on_message(client, userdata, msg):
                ...
                try:
                    ...
                    logger.info(f"✅ MQTT recibido: {msg.topic}")
                except Exception as e:
                    logger.error(f"❌ Error decodificando MQTT: {e}")
            ...
            if mensaje_recibido["data"]:
                ...
                logger.info(f"✅ Datos MQTT recuperados: {mensaje_recibido['data']}")
            else:
                logger.warning("⚠️  No hay datos MQTT en este momento")
                
        except ImportError:
            logger.warning("⚠️  paho-mqtt no instalado (MQTT no disponible)")
        except Exception as e:
            logger.warning(f"⚠️  No se pudo conectar a MQTT: {e}")
    ...
    def obtener_datos_cloud_ecowitt(self):
        ...
        if not (api_key and api_secret and device_id):
            logger.warning("⚠️  Credenciales Ecowitt Cloud no encontradas")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"✅ Datos Cloud recuperados: {len(data)} registros")
            else:
                logger.warning(f"⚠️  Cloud API retornó {response.status_code}: {response.text}")
                
        except ImportError:
            logger.warning("⚠️  requests no instalado (Cloud API no disponible)")
        except Exception as e:
            logger.warning(f"⚠️  No se pudo conectar a Cloud: {e}")
    ...
    def persistir_datos_recuperados(self):
        ...
        try:
            ...
            logger.info(f"✅ Datos guardados en: {archivo_historico}")
            ...
            try:
                ...
                logger.info("✅ Datos publicados en bus del sistema")
            except Exception as e:
                logger.warning(f"⚠️  No se pudo publicar en bus: {e}")
    ...
    def generar_reporte(self):
        ...
        logger.info("\n" + "="*80)
        logger.info("📊 REPORTE DE RECUPERACIÓN DE DATOS")
        logger.info("="*80)
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("🚀 RECUPERADOR DE DATOS HISTÓRICOS - ECOWITT HP2550A")
    print("="*80 + "\n")
    ...
    if archivo:
        ...
        print("❌ No se pudo persistir los datos")
