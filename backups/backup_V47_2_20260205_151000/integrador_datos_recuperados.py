...
def integrar_datos_recuperados(ruta_archivo: str):
    ...
    logger.info("\n" + "="*80)
    logger.info("🔄 INTEGRADOR DE DATOS RECUPERADOS")
    logger.info("="*80 + "\n")
    ...
    try:
        ...
        if resultado['status'] != 'exito':
            logger.error(f"❌ Error cargando datos: {resultado.get('errores')}")
            return False
    ...
    logger.info("\n📊 FASE 2: INFORMACIÓN DE RECUPERACIÓN\n")
    ...
    # 3. Guardar histórico
    logger.info("\n💾 FASE 3: GUARDANDO EN HISTÓRICO DEL SISTEMA\n")
    ...
    try:
        ...
        logger.info(f"✅ Sensores integrados: {len(sensores)} registros")
        logger.info(f"   Archivo: {archivo_sensores.name}")
    except Exception as e:
        logger.error(f"❌ Error guardando sensores: {e}")
    ...
    try:
        ...
        for evento in eventos[:5]:  # Primeros 5 como ejemplo
            try:
                ...
                logger.warning(f"⚠️  Error publicando evento: {e}")
        
        logger.info(f"✅ {len(eventos)} eventos publicados en el bus")
        ...
        if sensores:
            ...
            logger.info("✅ Estado de recuperación publicado en el bus")
        
    except ImportError as e:
        logger.warning(f"⚠️  SystemManager no disponible: {e}")
    ...
    logger.info("\n✅ Los datos están listos para usar en índices y análisis histórico\n")
...
if __name__ == "__main__":
    ...
    if not archivos:
        logger.error("❌ No se encontraron archivos de recuperación")
        sys.exit(1)
