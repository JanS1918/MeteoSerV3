...
def main():
    ...
    if not datos:
        print("❌ No hay datos para hacer duelos")
        return
    ...
    print(f"✅ {len(duelos_simulados)} duelos simulados")
    ...
    print(f"\n✅ Decisiones correctas:  {correctos}/{len(duelos_simulados)} ({correctos*100//len(duelos_simulados)}%)")
    print(f"❌ Decisiones incorrectas: {incorrectos}/{len(duelos_simulados)} ({incorrectos*100//len(duelos_simulados)}%)")
    
    if correctos == len(duelos_simulados):
        print(f"\n✅ EXCELENTE - Todas las decisiones son coherentes")
    elif correctos >= len(duelos_simulados) * 0.7:
        print(f"\n⚠️  ACEPTABLE - Mayoría de decisiones son correctas")
    else:
        print(f"\n❌ PROBLEMA - Muchas decisiones son incoherentes")
    ...
    print(f"✅ Reporte guardado en data/test_duelos_reporte.json")
