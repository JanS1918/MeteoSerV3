...
class AutoChangeWatchdog:
    ...
    def validate_spec_compliance(
        ...
        if non_canonical_spec:
            logger.warning(
                "⚠️ Spec con parámetros no canónicos para %s: %s",
                formula_name,
                non_canonical_spec,
            )
        if non_canonical_impl:
            logger.warning(
                "⚠️ Implementación con parámetros no canónicos para %s: %s",
                formula_name,
                non_canonical_impl,
            )
        ...
        if issues:
            for issue in issues:
                logger.warning(
                    "⚠️ MAXIMA: No auto-corregible en %s → %s",
                    formula_name,
                    issue,
                )
    ...
    def _freeze_watchdog(self, reason: str, duration_seconds: int = 12 * 3600):
        ...
        logger.warning(f"⛔ WATCHDOG CONGELADO por {duration_seconds}s")
