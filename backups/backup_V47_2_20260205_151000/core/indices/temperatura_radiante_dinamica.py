...
2. Radiación IR del cielo (VDI 3787)
...
def _calcular_punto_rocio_magnus(temp_aire_c: float, humedad_relativa_pct: float) -> float:
    """Punto de rocío con Magnus (entrada en °C, salida en °C)."""
    hr = max(1e-6, min(100.0, humedad_relativa_pct)) / 100.0
    a = 17.625
    b = 243.04
    gamma = math.log(hr) + (a * temp_aire_c) / (b + temp_aire_c)
    return (b * gamma) / (a - gamma)


def calcular_temperatura_cielo_vdi3787(
    temp_aire_c: float,
    nubosidad_pct: float = 0.0,
    temp_rocio_c: Optional[float] = None,
    humedad_relativa_pct: Optional[float] = None
) -> float:
    """
    Temperatura efectiva del cielo según VDI 3787.
    
    Usa emisividad de cielo claro dependiente de punto de rocío y
    corrección por nubosidad.
    """
    if temp_rocio_c is None and humedad_relativa_pct is not None:
        temp_rocio_c = _calcular_punto_rocio_magnus(temp_aire_c, humedad_relativa_pct)
    if temp_rocio_c is None:
        temp_rocio_c = temp_aire_c

    T_aire_k = temp_aire_c + 273.15
    n = max(0.0, min(1.0, nubosidad_pct / 100.0))
    ...
    lw_cielo = epsilon_sky * STEFAN_BOLTZMANN * (T_aire_k ** 4)
    T_cielo_k = (lw_cielo / STEFAN_BOLTZMANN) ** 0.25
    return T_cielo_k - 273.15
...
def calcular_temperatura_cielo_swinbank(
    ...
    """
    Alias de compatibilidad: VDI 3787 sustituye el cálculo anterior.
    """
    return calcular_temperatura_cielo_vdi3787(temp_aire_c, nubosidad_pct)


def calcular_temperatura_suelo_acoplada(
    temp_aire_c: float,
    radiacion_solar_w_m2: float,
    albedo_entorno: float,
    velocidad_viento_ms: float,
    temp_suelo_prev_c: Optional[float] = None,
    inercia_suelo_h: float = 6.0,
    delta_t_h: float = 1.0,
    emisividad_suelo: float = 0.95,
    temp_cielo_k: Optional[float] = None
) -> tuple[float, float]:
    """Modelo acoplado de masa térmica de suelo con inercia."""
    T_aire_k = temp_aire_c + 273.15
    q_sw = max(0.0, radiacion_solar_w_m2) * (1.0 - albedo_entorno)
    q_lw = 0.0
    if temp_cielo_k is not None:
        q_lw = emisividad_suelo * STEFAN_BOLTZMANN * (temp_cielo_k ** 4 - T_aire_k ** 4)

    h_c = 5.8 + 4.1 * max(0.1, velocidad_viento_ms)
    h_r = 4.0 * emisividad_suelo * STEFAN_BOLTZMANN * (T_aire_k ** 3)
    T_eq_c = temp_aire_c + (q_sw + q_lw) / (h_c + h_r)

    if temp_suelo_prev_c is None:
        temp_suelo_prev_c = temp_aire_c

    alpha = 1.0 - math.exp(-max(0.05, delta_t_h) / max(0.5, inercia_suelo_h))
    temp_suelo_c = temp_suelo_prev_c + alpha * (T_eq_c - temp_suelo_prev_c)
    return temp_suelo_c, T_eq_c
...
def calcular_temperatura_radiante_media(
    ...
    # 1. TEMPERATURA DEL CIELO (VDI 3787 + nubosidad)
    # ============================================================
    T_cielo_c = calcular_temperatura_cielo_vdi3787(
        temp_aire_c,
    ...
    if temp_suelo_c is None:
        temp_suelo_c, temp_suelo_eq_c = calcular_temperatura_suelo_acoplada(
            temp_aire_c=temp_aire_c,
            radiacion_solar_w_m2=radiacion_solar_w_m2,
            albedo_entorno=albedo_entorno,
            velocidad_viento_ms=velocidad_viento_ms,
            temp_suelo_prev_c=temp_suelo_prev_c,
            inercia_suelo_h=inercia_suelo_h,
            delta_t_h=delta_t_h,
            emisividad_suelo=emisividad_suelo,
            temp_cielo_k=T_cielo_k
        )
    else:
        temp_suelo_eq_c = temp_suelo_c
    ...
    return {
        ...
        "t_suelo_eq": round(temp_suelo_eq_c, 2),
