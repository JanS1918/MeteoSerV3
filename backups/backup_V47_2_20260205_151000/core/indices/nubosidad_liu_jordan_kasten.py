...
V45.0: VECTORIZACIÓN NUMPY para Carmona + Dilley & O'Brien radiativo nocturno.
...
- Dilley, A.C. & O'Brien, D.M. (1998). "Estimating downward clear sky
...
V45.0: Vectorización Matricial Completa
"""
...
def calcular_nubosidad_liu_jordan_kasten(
    ...
    Args:
        ...
        elevacion_lunar_deg: [V43.0] Elevación lunar aparente (grados)
        iluminacion_lunar_pct: [V43.0] Iluminación lunar (%)
    
    Returns:
        Dict con:
        - nubosidad: Fracción nubosa (0-100%)
        - kt_indice_claridad: Índice de Claridad K_t (0-1.2) o K_t,night (nocturno)
        ...
        - [V43.0] nubosidad_lunar: Componente validada con luz lunar
    ...
    Referencias:
        ...
        - [V43.0] Meeus (1998): Lunar position for night validation
    """
    # Validaciones
    if radiacion_extraterrestre_w_m2 < 10 or elevacion_solar_deg < 5:
        # Noche o sol muy bajo: usar indicadores atmosféricos + validación lunar
        return _calcular_nubosidad_nocturna(
            temp_aire_c, 
            temp_rocio_c, 
            humedad_relativa_pct,
            elevacion_lunar_deg,    # V43.0
            iluminacion_lunar_pct   # V43.0
        )
...
def _calcular_nubosidad_nocturna(
    ...
    """
    Estimación de nubosidad nocturna (Carmona + Dilley & O'Brien, LW).
    
    ⚛️ V43.0 VALIDACIÓN LUNAR:
    Si elevacion_lunar_deg e iluminacion_lunar_pct se proporcionan, calcula un
    Índice de Claridad Nocturno (K_t,night) basado en la luz lunar para auditar
    si hay nubes de noche. La luna actúa como "Sol débil" y permite detección
    radiométrica nocturna.
    
    Usa indicadores radiativos y atmosféricos:
    - Dilley & O'Brien: radiación infrarroja descendente en cielo despejado
    - Carmona: fracción nubosa desde humedad y depresión del punto de rocío
    - [V43.0] Luz lunar (elevación + iluminación) para validar nubosidad
    ...
    Returns:
        Dict con nubosidad nocturna estimada + K_t,night si hay luna visible
    """
    # V45.0: VECTORIZACIÓN - Calcular múltiples parámetros en arrays
    delta_t_dew = np.clip(temp_aire_c - temp_rocio_c, 0.0, 15.0)
    
    # Presión de vapor vectorizada (Magnus-Tetens)
    # e = 6.112 * exp((17.67 * T) / (T + 243.5))
    e_hpa = 6.112 * np.exp((17.67 * temp_rocio_c) / (temp_rocio_c + 243.5))
    e_pa = e_hpa * 100.0
    ...
    # Carmona VECTORIZADO: índice nuboso nocturno
    rh = np.clip(humedad_relativa_pct, 0.0, 100.0)
    factor_rh = (rh / 100.0) ** 1.3
    factor_dew = np.exp(-0.25 * delta_t_dew)
    indice_carmona = np.clip(0.6 * factor_rh + 0.4 * factor_dew, 0.0, 1.0)
    
    # Emisividad efectiva del cielo con cobertura nubosa (N^2)
    epsilon_sky = epsilon_clear + (1.0 - epsilon_clear) * indice_carmona
    lw_sky = epsilon_sky * sigma * (T_air_k ** 4)
    
    # Fracción nubosa desde emisividad
    denom = np.maximum(1e-6, 1.0 - epsilon_clear)
    cloud_frac = np.sqrt(np.clip((epsilon_sky - epsilon_clear) / denom, 0.0, 1.0))
    nubosidad_lw = float(cloud_frac * 100.0)
    
    nubosidad_atmosferica = nubosidad_lw

    # ⚛️ V43.0: VALIDACIÓN CON ARCO LUNAR (V45.0 VECTORIZADO)
    kt_night = 0.0
    nubosidad_lunar = nubosidad_lw
    confianza = 65.0

    if elevacion_lunar_deg is not None and iluminacion_lunar_pct is not None and elevacion_lunar_deg > 0:
        # Factores lunares vectorizados
        factor_vec = np.array([
            np.clip(elevacion_lunar_deg / 90.0, 0.0, 1.0),
            np.clip(iluminacion_lunar_pct / 100.0, 0.0, 1.0)
        ], dtype=np.float64)
        factor_lunar = float(np.prod(factor_vec))  # producto vectorizado

        if factor_lunar > 0.15:
            kt_night = factor_lunar * (1.0 - cloud_frac)
            nubosidad_lunar = 0.75 * nubosidad_lw + 0.25 * (1.0 - kt_night) * 100.0
            confianza = 80.0
    
    # Clasificación de cielo nocturno
    if nubosidad_lunar < 25:
        tipo_cielo = "Despejado (noche)"
    elif nubosidad_lunar < 60:
        tipo_cielo = "Parcialmente nublado (noche)"
    else:
        tipo_cielo = "Nublado/Cubierto (noche)"
    
    return {
        "nubosidad": round(nubosidad_lunar, 1),
        "kt_indice_claridad": round(kt_night, 3),  # V43.0: K_t,night
        "masa_aire": 0.0,
        "tipo_cielo": tipo_cielo,
        "nubosidad_radiometrica": round(nubosidad_lw, 1),
        "nubosidad_atmosferica": round(nubosidad_atmosferica, 1),
        "nubosidad_lunar": round(nubosidad_lunar, 1),  # V43.0
        "validacion_lunar": elevacion_lunar_deg is not None and elevacion_lunar_deg > 0,  # V43.0
        "confianza": confianza,
        "elevacion_solar": 0.0,
        "elevacion_lunar_deg": round(elevacion_lunar_deg, 2) if elevacion_lunar_deg else 0.0,  # V43.0
        "iluminacion_lunar_pct": round(iluminacion_lunar_pct, 2) if iluminacion_lunar_pct else 0.0,  # V43.0
        "delta_t_dew": round(delta_t_dew, 1),
        "lw_cielo_despejado": round(lw_clear, 2),
        "lw_cielo": round(lw_sky, 2),
        "emisividad_cielo_despejado": round(epsilon_clear, 4),
        "emisividad_cielo": round(epsilon_sky, 4),
        "indice_carmona": round(indice_carmona, 4)
    }
