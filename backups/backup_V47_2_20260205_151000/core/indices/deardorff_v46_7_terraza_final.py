...
"""
════════════════════════════════════════════════════════════════════════════════
🛡️ DEARDORFF V46.8 - SOBERANÍA ABSOLUTA NARCÍS MONTURIOL 36
════════════════════════════════════════════════════════════════════════════════
...
CONSTANTES_TERRAZA = {
    ...
    # Horizonte topográfico (SRTM verificado)
    "horizonte_minimo_grados": 8.5,
    "horizonte_maximo_grados": 9.2,
    "horizonte_promedio_grados": 8.85,
    "ocaso_topografico_adelanto_min": 38,  # Minutos antes de ocaso astronómico
    
    # SUELO - Sauló granítico (NOT asfalto)
    "tipo_suelo": "sauló_granito",
    "conductividad_suelo_wm2k": 2.2,  # ← ELEVADO (era 1.85)
    "capacidad_calorica_suelo": 1200000,  # J/(m³·K)
    "porosidad_suelo": 0.42,
    "profundidad_efectiva_m": 0.5,
    
    # SUPERFICIE - Baldosa Roja (Rasilla Catalana)
    "tipo_superficie": "ceramica_rasilla",
    "conductividad_rasilla_wm2k": 1.0,  # Baja conductividad, retiene calor
    "capacidad_calorica_rasilla": 1800000,  # J/(m³·K)
    "espesor_rasilla_m": 0.01,  # 1 cm típico
    "albedo_rasilla": 0.30,  # Rojo oscuro + polvo catalán absorbe MÁS
    "emisividad_rasilla": 0.92,  # Alta emisión térmica
    
    # MURO - Ladrillo/Hormigón + Efecto Chimenea
    "altura_muro_m": 2.5,  # Típico muro de terraza
    "altura_mastil_sobre_terraza_m": 1.2,  # Sensor está a 3.7m del suelo (112m + 1.2m)
    "distancia_edificio_vecino_m": 3.0,  # 3 metros de separación (trinchera)
    "factor_conveccion_chimenea_wm2": 1.0,  # Aire caliente sube
    
    # INERCIA TÉRMICA DEL MURO - EXPONENCIAL (Ley de Newton)
    "retorno_termico_muro_base_wm2": 0.3,  # Base en día nublado
    "factor_radiacion_dia": 0.0005,  # Multiplicador por radiación diurna
    "tau_inercia_h": 4.0,  # Constante de tiempo exponencial (decae en 4h)
    
    # REFLEXIÓN DE RADIACIÓN (LW atrapada entre edificios)
    "factor_vision_hacia_muro": 0.35,  # View Factor del edificio vecino
    "factor_vision_cielo": 0.65,  # View Factor del cielo (95°)
    "emisividad_muro_ladrillo": 0.90,
    "reflexion_lw_entre_muros": 0.15,  # Multiplicador de intensidad LW
    
    # BOSQUE (Cal Peix + Can Gallart a 500-1500m)
    "factor_radiacion_bosque_wm2": -0.8,  # Pérdida por sumidero térmico forestal
    "distancia_bosque_m": 800,  # Promedio
    
    # VIENTO - Gryning/M-O
    "velocidad_viento_min_calma_ms": 0.5,
    "multiplicador_estabilidad_calma": 1.10,  # +10% en calma
    
    # ADN GEOGRAFÍA - SELLO FINAL
    "ubicacion_humana": "Calle Narcís Monturiol 36, Argentona, Barcelona",
    "tipo_emplazamiento": "Terraza de piso urbano con efecto chimenea",
}
...
def test_suite():
    ...
    logger.info("\n" + "="*80)
    logger.info("✅ TEST SUITE COMPLETADO")
    logger.info("="*80 + "\n")
