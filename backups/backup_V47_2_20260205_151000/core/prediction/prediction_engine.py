...
def obtener_motor_prediccion(bus=None):
    ...
    def predecir(self) -> Dict[str, Any]:
        ...
        # Predicción de lluvia local (física: CAPE/LI/LCL/Qnet)
        ...
        if humedad is not None and presion is not None:
            ...
            punto_rocio = None
            if temp is not None:
                punto_rocio = temp - ((100 - humedad) / 5.0)

            cape_jkg = 0.0
            lcl_m = None
            if temp is not None and punto_rocio is not None:
                cape_result = calcular_cape(
                    temperatura_c=float(temp),
                    temperatura_rocio_c=float(punto_rocio),
                    presion_hpa=float(presion),
                )
                cape_jkg = float(cape_result.get("cape_jkg", 0.0))
                lcl_m = cape_result.get("lcl_m")

            lcl_km = (float(lcl_m) / 1000.0) if lcl_m is not None else max(0.0, (temp - punto_rocio) * 0.125) if temp is not None and punto_rocio is not None else 0.0
            t500_env = (temp - 6.5 * 5.5) if temp is not None else 0.0
            if lcl_km >= 5.5:
                t500_parcela = temp - 9.8 * 5.5 if temp is not None else 0.0
            else:
                t500_parcela = temp - 9.8 * lcl_km - 6.0 * (5.5 - lcl_km) if temp is not None else 0.0
            li = t500_env - t500_parcela

            # Qnet desde índices o estimado con nubosidad
            qnet_val = None
            if isinstance(indices, dict) and isinstance(indices.get("radiacion_neta"), dict):
                qnet_val = indices.get("radiacion_neta", {}).get("valor")
            if qnet_val is None and temp is not None:
                nub = None
            ...
            cape_factor = min(1.0, cape_jkg / 2000.0) if cape_jkg > 0 else 0.0
            li_factor = min(1.0, max(0.0, (-li) / 8.0))
            lcl_factor = 1.0 - min(1.0, (float(lcl_m) / 2500.0)) if lcl_m is not None else 0.0
            qnet_factor = 0.0
            if qnet_val is not None:
                qnet_factor = min(1.0, max(0.0, (float(qnet_val) - 50.0) / 200.0))

            prob_lluvia = 100.0 * (
                0.45 * cape_factor + 0.25 * li_factor + 0.20 * lcl_factor + 0.10 * qnet_factor
            )
            ...
            prob_lluvia = max(0.0, min(100.0, prob_lluvia))
        ...
        if utci_val is not None:
            ...
            support = self._support_factor(["temperatura", "humedad", "viento", "radiacion"])
            support *= self._support_from_indices(indices, ["utci"])
...
PredictionEngine = MotorPrediccion
