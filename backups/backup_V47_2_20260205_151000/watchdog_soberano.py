...
class WatchdogSoberano:
    ...
    def __init__(self, port: int = 9617, app_pid: Optional[int] = None, 
        ...
        logger.info(f"🛡️ Centinela Soberano inicializado (puerto {port}, heartbeat {heartbeat_interval}s, timeout {timeout}s)")
    ...
    def find_app_pid(self) -> Optional[int]:
        ...
        try:
            ...
            logger.warning(f"⚠️ No se pudo encontrar main app: {e}")
        return None
    ...
    def restore_snapshot(self) -> bool:
        ...
        try:
            ...
            if not snapshots:
                logger.warning("⚠️ No hay snapshots para restaurar")
                return False
    ...
    def kill_app(self, reason: str) -> bool:
        ...
        if not self.app_pid:
            logger.warning("⚠️ PID de app no disponible, no se puede matar")
            return False
        ...
        try:
            ...
            try:
                ...
                logger.error(f"❌ App aún viva después de SIGKILL!")
                return False
            except ProcessLookupError:
                logger.info(f"✅ App muerta: {self.app_pid}")
                return True
        except Exception as e:
            logger.error(f"❌ Error matando app: {e}")
            return False
    ...
    def setup_socket(self) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ Socket escuchando en puerto {self.port}")
            return True
        except Exception as e:
            logger.error(f"❌ Error configurando socket: {e}")
            return False
    ...
    def wait_for_heartbeat(self) -> bool:
        ...
        try:
            ...
            if data.strip() == "HEARTBEAT":
                ...
                logger.warning(f"⚠️ Datos inválidos recibidos: {data}")
                return False
                
        except socket.timeout:
            elapsed = (datetime.now() - self.last_heartbeat).total_seconds()
            logger.critical(f"🚨 TIMEOUT: Sin heartbeat por {elapsed:.1f}s (umbral {self.timeout}s)")
            return False
        except Exception as e:
            logger.error(f"❌ Error recibiendo heartbeat: {e}")
            return False
    ...
    def run(self):
        ...
        if not self.app_pid:
            logger.error("❌ No se puede obtener PID de app, abortando")
            return
        
        # Configurar socket
        if not self.setup_socket():
            logger.error("❌ No se puede configurar socket, abortando")
            return
        
        logger.info(f"🔄 Iniciando loop de monitoreo (app PID {self.app_pid})")
            ...
            try:
                ...
                if self.wait_for_heartbeat():
                    ...
                    if killed:
                        ...
                        logger.critical("🛡️ Centinela completó secuencia de emergencia")
