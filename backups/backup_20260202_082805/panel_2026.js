...
async function cargarPanelSuperior() {
    try {
        ...
        document.getElementById('ubicacion-info').textContent = 
            `📍 ${data.ubicacion.poblacion} (${data.ubicacion.latitud}°, ${data.ubicacion.longitud}°)`;
...
function renderizarArcoSolarHUD(datosSol) {
    ...
    path.setAttribute('d', 'M 10 70 Q 100 15, 190 70');
    ...
    horizonte.setAttribute('y1', '70');
    horizonte.setAttribute('x2', '190');
    horizonte.setAttribute('y2', '70');
    ...
    const posX = 10 + (190 - 10) * posicion;
    const posY = 70 - 55 * Math.sin(Math.PI * posicion);  // Parábola de elevación
...
function renderizarArcoLunarHUD(datosLuna) {
    ...
    // Arco de trayectoria lunar - viewBox 200x100
    const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    path.setAttribute('d', 'M 10 70 Q 100 15, 190 70');
    path.setAttribute('stroke', colorLinea);
    path.setAttribute('stroke-width', strokeWidth);
    ...
    const posX = 10 + (190 - 10) * posicion;
    const posY = 70 - 55 * Math.sin(Math.PI * posicion);
...
function crearCajonesPlaceholder(total, offset) {
    ...
    return nombres.slice(offset, offset + total).map((nombre, idx) => ({
        ...
        nombre,
        icono: '—',
        valores_tapa: [{ nombre: '—', valor: 0, unidad: '' }],
        valores_completos: []
    }));
}
