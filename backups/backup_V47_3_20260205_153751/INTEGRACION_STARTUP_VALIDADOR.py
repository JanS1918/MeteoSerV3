...
async def validate_formulas_on_startup():
    ...
    logger.info("🔍 INICIANDO VALIDACIÓN PROACTIVA DE FÓRMULAS...")
    ...
    logger.info(f"📊 Validando {len(implementations)} fórmulas...")
    validation_results = {}
    ...
    for formula_name, impl_func in implementations.items():
        ...
        if not is_compliant:
            logger.warning(
                f"⚠️  {formula_name} incumple especificación: {issues}"
            )
        else:
            logger.info(f"✅ {formula_name} validada correctamente")
    ...
    if fatal_errors > 0:
        logger.critical(
            f"🚨 {fatal_errors} FÓRMULAS CON ERRORES FATALES DETECTADOS"
        )
        ...
        logger.info(f"✅ TODAS LAS FÓRMULAS VALIDADAS EXITOSAMENTE")
    ...
    logger.info("✅ VALIDACIÓN PROACTIVA COMPLETADA")
...
async def propose_formula_change(change_request: ChangeRequest):
    ...
    logger.info(f"✅ Cambio de {formula_name} pasó validación proactiva")
...
INSTALL_CHECKLIST = """
✅ Instalaciones Manuales Necesarias:
...
2. En auto_change_watchdog.py:
   - ✅ HECHO: validate_spec_compliance() ya existe
   - ✅ HECHO: block_noncompliant_change() ya existe
   - ✅ HECHO: _freeze_watchdog() ya existe

3. En spec_validation_engine.py:
   - ✅ HECHO: SpecValidationEngine clase completa
...
2. Buscar en logs:
   "🔍 INICIANDO VALIDACIÓN PROACTIVA"
   "✅ TODAS LAS FÓRMULAS VALIDADAS EXITOSAMENTE"
   
   O si hay errores:
   "⚠️  [formula] incumple especificación"
   "🚨 X FÓRMULAS CON ERRORES FATALES"
