    ...
    def validate(self, 
        ...
        if formula_normalized in self.METEOROLOGICAL_INDICES:
            ...
            if source_library in self.ALLOWED_LIBRARIES:
                ...
                return False, f"❌ BLOQUEADA: {source_library}.{source_function} ({blocked_info['reason']})"
            
            else:
                # ⚠️ Librería desconocida - evaluar por función
                if source_function in ["erf", "gamma", "special"]:
                    return False, f"❌ BLOQUEADA: {source_library}.{source_function} (función matemática, no meteorológica)"
                ...
                return True, f"✅ PERMITIDA: {formula_name}"
        
        else:
            # ❌ Nombre NO es índice conocido
            ...
            if source_library in self.BLOCKED_LIBRARIES:
                ...
                return False, f"❌ BLOQUEADA: {source_library}.{source_function} no es índice meteorológico"
            
            else:
                # ⚠️ Nombre desconocido - evaluar riesgo
                if source_library in ["scipy", "numpy", "math"]:
                    ...
                    return False, f"⚠️ BLOQUEADA: '{formula_name}' NO es índice meteorológico conocido (en {source_library})"
                    ...
                    return True, f"⚠️ PERMITIDA CON ADVERTENCIA: '{formula_name}' no es índice conocido pero librería ({source_library}) es permitida"
...
if __name__ == "__main__":
    ...
    print("\n📊 LOG DE RECHAZOS:", len(validator.get_rejection_log()))
    print("📊 LOG DE ACEPTACIONES:", len(validator.get_acceptance_log()))
