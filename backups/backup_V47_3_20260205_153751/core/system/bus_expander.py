...
class BusExpander:
    ...
    def __init__(self, bus, system):
        ...
        try:
            ...
            logger.warning("⚠️ RollingWindowManager no disponible, ventanas desactivadas")
        ...
        try:
            ...
            if self.rolling_windows:
                try:
                    ...
                    logger.debug("✅ V45.0: Ventanas rolling actualizadas y publicadas al Bus")
                except Exception as rolling_err:
                    logger.warning(f"⚠️ Error en rolling windows: {rolling_err}")
            
            logger.info("✅ BUS V45.0 VECTORIZADO: 2000+ CONSTANTES - POTENCIA MATRICIAL ACTIVA - NUMPY ACELERADO 🎯⚡")
    ...
    async def _publish_astronomia(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"  ⚠️ Error calculando arco lunar: {e_luna}")
            
            logger.info("✅ Astronomía publicada (SPA solar + Meeus lunar + 20 subfactores)")
    ...
    async def _publish_alertas_meteorologicas(self):
        ...
        try:
            ...
            # 8. ALERTA DESCARGAS ELÉCTRICAS (validación local - sin HTTP externo)
            # NOTA: validación externa removida (pureza de seguridad local)
            hay_rayos = False  # Fallback local: solo usa sensores internos
    ...
    async def _publish_confort_avanzado(self):
        ...
        try:
            ...
            logger.info(f"✅ Confort avanzado publicado (PMV:{pmv:.1f}, WBGT:{wbgt:.1f}°C, CAPE:{cape_val if 'cape_val' in locals() else None}J/kg)")
    ...
    async def _publish_humedad_suelo_et(self):
        ...
        try:
            ...
            # ═══════════════════════════════════════════════════════════
            # 🛡️ V47.0 KALMAN SOIL WH51 - FILTRO DE RUIDO
            # ═══════════════════════════════════════════════════════════
