...
try:
    ...
    print("⚠️ influxdb-client no disponible. Instalar con: pip install influxdb-client")
...
class InfluxDBManager:
        ...
        for intento in range(reintentos):
            try:
                ...
                if health.status == "pass":
                    ...
                    print(f"✅ Conectado a InfluxDB ({self.url})")
                    return True
                else:
                    print(f"⚠️  InfluxDB health check failed: {health.message}")
            
            except Exception as e:
                print(f"❌ Error conectando a InfluxDB (intento {intento+1}/{reintentos}): {e}")
                time.sleep(2)
    ...
    def escribir_punto(self, measurement: str, fields: Dict[str, Any],
        ...
        if not self.conectado:
            print("⚠️  No conectado a InfluxDB")
            return
        ...
        try:
            ...
            print(f"❌ Error escribiendo punto: {e}")
    ...
    def crear_bucket(self, nombre: str, retencion_dias: int = 30) -> bool:
        ...
        try:
            ...
            if any(b.name == nombre for b in buckets):
                print(f"⚠️  Bucket '{nombre}' ya existe")
                return True
            ...
            print(f"✅ Bucket '{nombre}' creado (retención: {retencion_dias} días)")
            return True
