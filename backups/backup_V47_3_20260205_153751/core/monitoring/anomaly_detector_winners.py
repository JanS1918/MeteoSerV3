...
class AnomalyDetectorWinners:
    ...
    def analyze_improvement(self, formula_name: str, improvement_pct: float,
        ...
        if is_overfitting:
            reason = f"❌ OVERFITTING: Mejora {improvement_pct:.1%} pero estabilidad cae {stability_delta:.1%}"
        else:
            if improvement_pct <= self.MIN_IMPROVEMENT:
                reason = f"✅ Mejora baja ({improvement_pct:.1%}), no sospechosa"
            elif stability_delta >= self.MAX_STABILITY_LOSS:
                reason = f"✅ Estabilidad estable ({stability_delta:.1%}), mejora legítima"
            else:
                reason = f"✅ Mejora {improvement_pct:.1%} con estabilidad {stability_delta:.1%}, válida"
