...
class CascadeDepthGate:
    ...
    def __init__(self):
        ...
        logger.info(f"✅ CascadeDepthGate inicializado (max depth: {self.MAX_DEPTH})")
    ...
    def extract_dependencies(self, code: str) -> Set[str]:
        ...
        try:
            ...
            logger.warning(f"⚠️ Error extrayendo dependencias: {e}")
    ...
    def calculate_depth(self, formula_name: str, visited: Set[str] = None, 
        ...
        if formula_name in visited:
            logger.warning(f"⚠️ Ciclo detectado en {formula_name}")
            return depth
    ...
    def analyze(self, formula_name: str, formula_code: str = None,
        ...
        # Generar razón
        if is_valid:
            reason = f"✅ Profundidad válida: {max_depth} ≤ {self.MAX_DEPTH}"
        else:
            if cascading_chains:
                chain_strs = [" → ".join(chain) for chain in cascading_chains[:3]]
                reason = f"❌ Cascada detectada (profundidad {max_depth}): {chain_strs[0]}"
                if len(cascading_chains) > 1:
                    reason += f" (+{len(cascading_chains)-1} más)"
            else:
                reason = f"❌ Profundidad excedida: {max_depth} > {self.MAX_DEPTH}"
    ...
    def validate_for_duelo(self, formula_name: str) -> Tuple[bool, str]:
        ...
        if analysis.is_valid:
            ...
            logger.warning(f"❌ {analysis.reason}")
...
if __name__ == "__main__":
    ...
    # stress_index: INVÁLIDA (profundidad 2) ❌
