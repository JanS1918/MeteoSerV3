...
def actualizar_sensores_ecowitt(data: dict):
    ...
    if viento is not None:
        try:
            ...
            system.actualizar_sensor("viento", viento_kmh)
        except Exception:
...
def estado():
    ...
    try:
        ...
        if "arco_solar" not in indices:
            ...
            arco_val = arco_solar(lat, hoy)
            indices["arco_solar"] = {"valor": arco_val, "estimado": estimado}
            indices["duracion_dia_h"] = {"valor": arco_val / 15.0, "estimado": estimado}
            if hasattr(system, "actualizar_indice"):
                system.actualizar_indice("arco_solar", arco_val)
                system.actualizar_indice("duracion_dia_h", arco_val / 15.0)
        try:
            import math
