...
def main():
    ...
    try:
        ...
        if duelos:
            print(f"✅ {len(duelos)} duelos ejecutados")
            ...
            print("⚠️  No se ejecutaron duelos (¿datos insuficientes?)")
            return
    
    except Exception as e:
        print(f"❌ Error ejecutando duelos: {e}")
        import traceback
        traceback.print_exc()
        return
    ...
    if not duelos_path.exists():
        print("⚠️  No hay histórico (¿recorders deshabilitados?)")
        return
    ...
    print(f"✅ {len(duelos_guardados)} duelos en histórico")
    ...
    for duelo in duelos_guardados:
        ...
        if score_a is None or score_b is None:
            print(f"   ⚠️  {param}: Sin scores")
            continue
        
        # Validación: A debe ser > B (A = ganador)
        if score_a > score_b:
            correctos += 1
            print(f"   ✅ {param:25} | A({score_a:.4f}) > B({score_b:.4f}) | {ganador}")
        elif score_a == score_b:
            print(f"   ⚠️  {param:25} | A({score_a:.4f}) == B({score_b:.4f}) | Empatado")
            errores += 1
        else:
            print(f"   ❌ {param:25} | A({score_a:.4f}) < B({score_b:.4f}) | ERROR INVERTIDO")
            errores += 1
    ...
    if total > 0:
        ...
        print(f"\n✅ Duelos correctos: {correctos}/{total} ({pct}%)")
        print(f"❌ Duelos con error: {errores}/{total}")
        
        if errores == 0:
            print(f"\n✅ PERFECTO - Todos los duelos son coherentes")
        else:
            print(f"\n⚠️  REVISAR - Hay duelos con problemas")
    else:
        print("⚠️  No hay duelos para validar")
