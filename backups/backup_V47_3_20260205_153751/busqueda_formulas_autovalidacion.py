...
class BuscadorFormulas:
    ...
    async def buscar_candidatas(self) -> None:
        ...
        """
        logger.info("="*80)
        logger.info("🔍 INICIANDO BÚSQUEDA DE FÓRMULAS CANDIDATAS")
        logger.info("="*80)
        ...
        for area in self.areas_criticas:
            if candidatas_aprobadas >= 5:
                logger.info(f"\n✅ OBJETIVO ALCANZADO: {candidatas_aprobadas} fórmulas aprobadas")
                break
            ...
            for candidata in candidatas:
                ...
                if not resultado_duelo['gana']:
                    logger.info(f"     ❌ RECHAZADA en duelo")
                ...
                logger.info(f"     ✅ PASA las 25 capas")
                ...
                logger.info(f"\n     🎯 FÓRMULA #{candidatas_aprobadas} APROBADA")
    ...
    async def generar_reporte(self) -> None:
        ...
        logger.info("\n\n" + "="*80)
        logger.info("📊 REPORTE FINAL - FÓRMULAS APROBADAS")
        logger.info("="*80)
        
        if not self.resultados:
            logger.warning("⚠️  No se encontraron fórmulas que cumplan todos los criterios")
            return
        
        logger.info(f"\n✅ Total de fórmulas aprobadas: {len(self.resultados)}")
        logger.info("\nDETALLE:")
        logger.info("-"*80)
...
async def main():
    ...
    try:
        ...
        logger.exception(f"❌ Error durante búsqueda: {e}")
