...
def nubosidad_romps_2017(temp_c: float, presion_kpa: float, humedad: float) -> Dict[str, float]:
    ...
    g_cape = GRAVEDAD.DINAMICA  # UNIFICADO: 9.80272394
    H_escala = (287.05 * T_k) / g_cape  # Usando gravedad real de Argentona
    ...
    if delta_t_lcl > 0.1:  # Parcela tiene flotabilidad positiva
        cape = g_cape * delta_t_lcl * (H_escala / 1000.0) / T_k  # Normalizado
