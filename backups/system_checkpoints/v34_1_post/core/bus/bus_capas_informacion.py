...
class BusCapasInformacion:
    ...
    def consumir_elite(self, parametro_abstracto: str, consumidor: str) -> tuple:
        ...
        jerarquia = FORMULA_HIERARCHY.get(parametro_abstracto)
        if not jerarquia:
            import logging
            logger = logging.getLogger("bus_omniscience")
            logger.error(
                f"❌ Parámetro '{parametro_abstracto}' NO está en Registro de Élite V30.1"
            )
            return (None, 0, None)
        ...
        logger.error(
            f"❌ {consumidor} pidió '{parametro_abstracto}' - "
    ...
    def publicar_elite(self, parametro_abstracto: str, valor: Any, 
        ...
        if not nivel_obj:
            logger.error(
                f"Nivel {nivel_elite} no definido para '{parametro_abstracto}' en Registro"
            )
            return
        ...
        logger.info(
            f"📤 {publicador} publicó '{parametro_abstracto}' "
