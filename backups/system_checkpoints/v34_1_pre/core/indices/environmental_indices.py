...
MANIFIESTO_PREDICCIONES_V20_SHA256 = "5abdbe9a44d92a99a4ca0186268601de982db1fd448aff78ca092b8bceae5b13"
...
def _dew_point(temp_c: float, rh_pct: float) -> float:
    ...
    LIMITACIONES:
    ==============
    ✅ Método Newton-Raphson: Cuadráticamente convergente
    ✅ Coeficientes NIST: Precisión ±0.1 Pa sobre rango amplio
    ✅ Cambio fase: Diferencia automática agua/hielo en T=0
    ✅ Derivada analítica: Evita diferencias numéricas
    ✅ Convergencia máxima: Precisión 1e-12 K garantizada
    ❌ Asume presión constante (validez ±0.01°C/100hPa)
    ❌ No valida para |HR| > 100% (clamping forcado)
    ❌ Asume composición aire constante (multicomponente despreciable)
    ...
    for iteration in range(20):
        ...
        if abs(d_ln_es_dT) < 1e-15:
            logger.warning(f"  ⚠️ Derivada demasiado pequeña: {d_ln_es_dT:.2e}, abortando iteración")
            break
        ...
        if abs(delta) < 1e-12:  # Máxima precisión
            logger.debug(f"  ✅ Punto rocío convergido: Td={td_guess:.3f}°C (iter={iteration+1}, |Δ|={abs(delta):.2e})")
            break
    else:
        logger.warning(f"  ⚠️ Newton-Raphson NO convergió después 20 iteraciones (último |Δ|={abs(delta):.2e})")
...
def _calcular_nubosidad_estimada(temp: float | None, dew: float | None, rh: float | None, viento: float | None,
    ...
    # Usar modelo físico Liu & Jordan + Kasten si hay radiación
    if es_dia and rad_teorica and rad_teorica > 50 and rad_real is not None:
        try:
            from core.indices.nubosidad_liu_jordan_kasten import calcular_nubosidad_liu_jordan_kasten
            
            # Calcular elevación solar aproximada
            # (idealmente vendría del Bus, pero fallback simplificado)
            elevacion_solar = 45.0  # Fallback (debería venir de astronomía)
            if rad_teorica > 0:
                # Aproximación: sin(elev) ≈ rad_teorica / 1367
            ...
            resultado_nub = calcular_nubosidad_liu_jordan_kasten(
                ...
                presion_hpa=1013.25,  # Asumido (debería venir del Bus)
                dia_ano=180  # Asumido (debería venir del contexto)
            )
            
            return resultado_nub["nubosidad"]
            
        except Exception as e:
            # Fallback a método empírico si falla
            pass
    
    # Fallback empírico (método anterior, solo si falla físico o es noche)
    ...
    nub_atmos = (
        0.6 * sat_factor +  # Aumentado de 0.4 (más peso a saturación)
        0.3 * rh_factor +
        0.1 * viento_factor  # Reducido de 0.2
    ) * 100
    
    return max(0, min(100, nub_atmos))
...
class EnvironmentalIndices:
    ...
    def evapotranspiracion_penman_monteith(self):
        ...
        return {
            ...
            "estimado": estimado,
            "explicacion": f"Penman-Monteith avanzada (Rn={round(rn, 2)}MJ/m2, V={v_val}m/s, VPD={round(vpd, 3)}kPa)",
        }
    ...
    def sensacion_termica(self, context=None):
        ...
        try:
            ...
            if self._bus:
                self._bus.publicar(
                    "utci",
                    float(utci_result["calle"]),
                    "indice_utci",
                    {
                        "humedad_fuente": "real" if humedad_es_real else "estimada",
                        "tmrt": utci_result.get("tmrt"),
                        "viento_corregido": utci_result.get("wind_calle"),
                    }
                )
                if utci_result.get("tmrt") is not None:
                    self._bus.publicar(
                        "temp_radiante_media",
                        float(utci_result["tmrt"]),
                        "indice_utci",
                        {"unidad": "C"}
                    )
                if utci_result.get("wind_calle") is not None:
                    self._bus.publicar(
                        "velocidad_viento_corregida",
                        float(utci_result["wind_calle"]),
                        "indice_utci",
                        {"unidad": "m/s"}
                    )
    ...
    def punto_rocio(self):
        ...
        # ⚡ CASCADA: Publicar en el bus como Wexler (Elite 5)
        if self._bus:
...
# ⚡ EUTANASIA TÉCNICA 2026: aproximaciones legacy ELIMINADAS
# Cascada modernizada:
...
def calcular_saturacion_vapor_con_fallback(temp_c: float, presion_pa: float = None) -> tuple:
    ...
    # ⚡ EUTANASIA TÉCNICA: aproximaciones legacy ELIMINADAS de cascada
    funciones = [
