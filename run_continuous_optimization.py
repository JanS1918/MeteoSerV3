...
class OptimizationScheduler:
    ...
    def ejecutar_una_vez(self) -> None:
        ...
        logger.info("[LAUNCH] Ejecutando ciclo de optimización (una sola vez)")
        try:
            ciclo = self.orchestrator.ejecutar_ciclo()
            self._log_resumen(ciclo)
        except Exception as e:
            logger.exception(f"[ERROR] Error en ciclo: {e}")

    def ejecutar_continuo(self) -> None:
        """Ejecuta ciclos periódicamente en background."""
        logger.info(f"[REINICIO] Iniciando modo continuo (intervalo: {self.intervalo_minutos} min)")
        ...
        try:
            while self.corriendo:
                ...
                logger.info(f"\n[FECHA] Próximo ciclo programado para: {self.proximo_ciclo.isoformat()}")
    ...
    def _log_resumen(self, ciclo) -> None:
        ...
        logger.info(f"\n[STATS] RESUMEN CICLO #{ciclo.ciclo_num}:")
        logger.info(f"   [OK] Parámetros procesados: {ciclo.parametros_procesados}")
        logger.info(f"   [BUSCAR] Candidatas descubiertas: {ciclo.candidatas_descubiertas}")
...
def main():
    ...
    logger.info("\n" + "=" * 70)
    logger.info("[TARGET] OPTIMIZADOR AUTOMÁTICO DE FÓRMULAS - SISTEMA CONTINUO")
    logger.info("=" * 70)
