...
"""
════════════════════════════════════════════════════════════════════════════════
[GUARDIAN] GUARDIÁN DE 25 CAPAS - AUDITORÍA TOTAL V47.0
════════════════════════════════════════════════════════════════════════════════
...
def capa_01_05_verificar_archivos() -> Dict:
    ...
    """
    logger.info("=" * 80)
    logger.info("[GUARDIAN] CAPAS 1-5: VERIFICACIÓN DE EXISTENCIA DE ARCHIVOS")
    logger.info("=" * 80)
    ...
    for categoria, formulas in CATALOGO_FORMULAS.items():
        ...
        for nombre_formula, datos in formulas.items():
            ...
            ruta = Path(archivo)
            if ruta.exists():
                logger.info(f"  [OK] {nombre_formula}: {archivo} (estado: {estado})")
                ...
                try:
                    ...
                    if hasattr(module, funcion):
                        logger.info(f"     [OK] Función {funcion}() verificada")
                        resultados["funciones_verificadas"].append({
                            "formula": nombre_formula,
                            "funcion": funcion,
                        })
                    else:
                        logger.warning(f"     [WARNING] Función {funcion}() NO ENCONTRADA en {archivo}")
                    ...
                    logger.warning(f"     [WARNING] Error verificando {funcion}(): {e}")
                    
            else:
                logger.error(f"  [ERROR] {nombre_formula}: {archivo} NO EXISTE")
                resultados["archivos_faltantes"].append({
                    "formula": nombre_formula,
                    "archivo": archivo,
                })
    
    logger.info(f"\n[STATS] RESUMEN CAPAS 1-5:")
...
def capa_06_10_analizar_uso_bus() -> Dict:
    ...
    """
    logger.info("\n" + "=" * 80)
    logger.info("[GUARDIAN] CAPAS 6-10: ANÁLISIS DE USO REAL EN BUS")
    logger.info("=" * 80)
    ...
    if not bus_file.exists():
        logger.error("[ERROR] bus_expander.py NO ENCONTRADO")
    ...
    for categoria, formulas in CATALOGO_FORMULAS.items():
        for nombre_formula, datos in formulas.items():
            ...
            if funcion in contenido_bus or nombre_formula.lower() in contenido_bus.lower():
                logger.info(f"  [OK] {nombre_formula}: EN USO en bus_expander.py")
                ...
                if estado in ["IMPLEMENTADO_V47", "DISPONIBLE"]:
                    logger.warning(f"  [WARNING] {nombre_formula}: DISPONIBLE pero NO USADA en bus")
    ...
    if resultados["formulas_disponibles_no_usadas"]:
        logger.warning(f"\n[WARNING] ALERTA: {len(resultados['formulas_disponibles_no_usadas'])} fórmulas superiores NO están en el Bus:")
...
def capa_11_15_duelo_formulas() -> Dict:
    ...
    """
    logger.info("\n" + "=" * 80)
    logger.info("[GUARDIAN] CAPAS 11-15: DUELO DE FÓRMULAS (Mejor vs Mejor)")
    logger.info("=" * 80)
        ...
        if len(formulas_ordenadas) > 1:
            ...
            if en_uso:
                ...
                if mejor_nombre != en_uso_nombre:
                    ...
                    logger.info(f"  [OK] {mejor_nombre} ya es la mejor y está en uso")
                ...
                logger.info(f"  [INFO] No hay fórmula marcada como 'EN_USO' en {categoria}")
    
    logger.info(f"\n[STATS] RESUMEN CAPAS 11-15:")
...
def capa_16_20_fusiones_posibles() -> Dict:
    ...
    """
    logger.info("\n" + "=" * 80)
    logger.info("[GUARDIAN] CAPAS 16-20: DETECCIÓN DE FUSIONES POSIBLES")
    logger.info("=" * 80)
    
    fusiones_propuestas = []
    
    # Fusión 1: Prata + Deardorff → Ya existe en V47.0
    logger.info("  [OK] Fusión Prata + Deardorff: YA IMPLEMENTADA (Deardorff V47.0)")
    ...
    logger.info("  [OK] Fusión Wright + FAO-56 PM: WRAPPER CREADO")
    fusiones_propuestas.append({
...
def capa_21_25_plan_accion(
    ...
    """
    logger.info("\n" + "=" * 80)
    logger.info("[GUARDIAN] CAPAS 21-25: PLAN DE ACCIÓN AUTOMÁTICO")
    logger.info("=" * 80)
...
def capa_00_validar_formulas_propias() -> Dict:
    ...
    """
    logger.info("[GUARDIAN] CAPA 0: PRE-AUDITORÍA - VALIDACIÓN DE FÓRMULAS PROPIAS")
    ...
    # Resumen
    logger.info(f"\n[STATS] RESUMEN CAPA 0:")
    logger.info(f"[OK] Fórmulas SANAS: {len(validaciones['formulas_sanas'])}")
    logger.info(f"[ERROR] Fórmulas DEFECTUOSAS: {len(validaciones['formulas_defectuosas'])}")
    logger.info(f"[WARNING] Fórmulas SOSPECHOSAS: {len(validaciones['formulas_sospechosas'])}")
    
    if validaciones["formulas_defectuosas"]:
        logger.error("[CRITICAL] ALERTA CRÍTICA: Fórmulas defectuosas detectadas!")
    ...
    if validaciones["formulas_sospechosas"]:
        logger.warning("[WARNING] Fórmulas sospechosas (revisar antes de duelos):")
                ...
                if resultado is not None:
                    if isinstance(resultado, (float, int)):
                        if resultado != resultado:  # NaN check
                            detalles.append("[WARNING] Devuelve NaN en rango normal")
                            estado = "SOSPECHOSA"
                        elif abs(resultado) > 1e10:
                            detalles.append(f"[WARNING] Valor muy grande: {resultado}")
                            estado = "SOSPECHOSA"
                    elif isinstance(resultado, dict):
                        # Verificar si hay NaN/Inf en dict
                        for k, v in resultado.items():
                            if isinstance(v, float) and (v != v or abs(v) > 1e10):
                                detalles.append(f"[WARNING] Salida {k}={v} es NaN/Inf")
                                estado = "SOSPECHOSA"
        except ZeroDivisionError:
            detalles.append("[ERROR] División por cero en rango normal")
            estado = "DEFECTUOSA"
        except TypeError as e:
            detalles.append(f"[WARNING] Error parámetros: {str(e)[:40]}")
            estado = "SOSPECHOSA"
        except Exception as e:
            detalles.append(f"[WARNING] Error en test normal: {str(e)[:40]}")
            estado = "SOSPECHOSA"
        
        # Test 2: Verificar firma (debe tener parámetros)
        if len(params) == 0:
            detalles.append("[ERROR] Función sin parámetros")
            estado = "DEFECTUOSA"
...
def capa_26_30_validar_duelos_y_decidir(
    ...
    """
    logger.info("\n[GUARDIAN] CAPAS 26-30: POST-DUELO - VALIDACIÓN DE DECISIONES")
    ...
    for cambio in capa_11_15_duelos.get("cambios_recomendados", []):
        ...
        if estado_actual == "DEFECTUOSA" or estado_candidata == "DEFECTUOSA":
            ...
            logger.error(f"[ERROR] Duelo descartado: fórmulas defectuosas")
            ...
            logger.warning(f"[WARNING] Duelo sospechoso: revisar antes de decidir")
            ...
            logger.info(f"[OK] Duelo confiable: {formula_candidata} gana con 95% confianza")
    
    # Resumen
    logger.info(f"\n[STATS] RESUMEN CAPAS 26-30:")
    logger.info(f"[OK] Duelos CONFIABLES: {len(duelos_validados['duelos_confiables'])}")
    logger.info(f"[WARNING] Duelos SOSPECHOSOS: {len(duelos_validados['duelos_sospechosos'])}")
    logger.info(f"[ERROR] Duelos DESCARTADOS: {len(duelos_validados['duelos_descartados'])}")
...
def ejecutar_guardian_25_capas():
    ...
    """
    logger.info("\n" + "=" * 80)
    logger.info("[GUARDIAN][GUARDIAN][GUARDIAN] GUARDIÁN DE 30 CAPAS - AUDITORÍA TOTAL V47.0 [GUARDIAN][GUARDIAN][GUARDIAN]")
    logger.info("=" * 80)
    ...
    logger.info(f"\n[GUARDIAN] CAPA 0 (PRE-AUDITORÍA):")
    logger.info(f"   [OK] Fórmulas SANAS: {len(capa_0['formulas_sanas'])}")
    logger.info(f"   [ERROR] Fórmulas DEFECTUOSAS: {len(capa_0['formulas_defectuosas'])}")
    logger.info(f"   [WARNING] Fórmulas SOSPECHOSAS: {len(capa_0['formulas_sospechosas'])}")
    
    logger.info(f"\n📚 CAPAS 1-25 (AUDITORÍA CLÁSICA):")
    logger.info(f"   [OK] Archivos verificados: {len(verificacion['archivos_encontrados'])}")
    logger.info(f"   [OK] Funciones verificadas: {len(verificacion['funciones_verificadas'])}")
    logger.info(f"   [WARNING] Fórmulas disponibles NO usadas: {len(uso_bus['formulas_disponibles_no_usadas'])}")
    ...
    if capa_0['formulas_defectuosas']:
        logger.error("\n[CRITICAL] CRÍTICO: Fórmulas defectuosas detectadas en CAPA 0")
        logger.error("[BLOQUEADO] NO ejecutar duelos hasta corregir:")
        for formula in capa_0['formulas_defectuosas']:
            logger.error(f"   - {formula}")
    
    if duelos_validados['duelos_descartados']:
        logger.warning("\n[WARNING] Algunos duelos fueron descartados (fórmulas no sanas)")
        for duelo in duelos_validados['duelos_descartados']:
            logger.warning(f"   - {duelo['comparacion']}")
    
    if plan['acciones_criticas']:
        logger.warning("\n[WARNING] ALERTA: HAY ACCIONES CRÍTICAS PENDIENTES")
        logger.warning("El sistema NO está en su estado SUMMUM óptimo.")
    else:
        logger.info("\n[OK] SISTEMA EN ESTADO SUMMUM ÓPTIMO")
