...
try:
    ...
    print(f"[OK] FormulaOptimizer instanciado correctamente")
    ...
    print(f"[ERROR] Error con FormulaOptimizer: {e}")
    sys.exit(1)
...
try:
    ...
    if hasattr(engine, '_optimizer'):
        print(f"[OK] FormulaDuelEngine tiene _optimizer")
        print(f"   Tipo: {type(engine._optimizer).__name__}")
    else:
        print(f"[ERROR] FormulaDuelEngine NO tiene _optimizer")
        sys.exit(1)
    ...
    if 'aplicar_mejora_simulada' in metodo_eval:
        print(f"[OK] _evaluar_formula usa aplicar_mejora_simulada")
    else:
        print(f"[ERROR] _evaluar_formula NO usa aplicar_mejora_simulada")
        sys.exit(1)
    
    print()
    
except Exception as e:
    print(f"[ERROR] Error verificando integración: {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)
...
try:
    ...
    if len(formulas_lista) < 2:
        print(f"[WARNING]  Solo {len(formulas_lista)} fórmula(s) disponible(s) en FORMULA_HIERARCHY")
        ...
        print(f"[OK] Motor de duelos LISTO")
...
print()
print("[OK] Seguridad y reversibilidad")
...
print()
print("="*80)
print("[TARGET] SISTEMA LISTO PARA DUELOS CON AUTO-MEJORA")
print("="*80)
