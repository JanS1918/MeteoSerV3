            ...
            try:
                ...
                if edad_dias <= 2:
                    with open(file, 'r', encoding='utf-8') as fp:
                        ...
                        logger.info(f"[OK] {file.name} ({file.stat().st_size} bytes)")
            except Exception as e:
                logger.warning(f"[WARNING]  {file.name}: {e}")
    
    logger.info(f"\n[STATS] Total recuperado: {len(datos_recuperados)} archivos")
...
def recuperar_historial_sensores():
    ...
    logger.info("\n" + "="*80)
    logger.info("[STATS] FASE 2: HISTÓRICO DE SENSORES")
    logger.info("="*80 + "\n")
    ...
    for file in sensor_files:
        try:
            with open(file, 'r', encoding='utf-8') as fp:
                ...
                logger.info(f"[OK] {file.name} ({len(data) if isinstance(data, list) else 1} registros)")
        except Exception as e:
            logger.warning(f"[WARNING]  {file.name}: {e}")
...
def persistir_datos_recuperados(datos_locales, historial_sensores):
    ...
    logger.info("\n" + "="*80)
    logger.info("[GUARDAR] FASE 3: PERSISTENCIA DE DATOS CONSOLIDADOS")
    logger.info("="*80 + "\n")
    ...
    try:
        ...
        logger.info(f"[OK] Datos guardados en: {archivo_consolidado}")
        ...
        logger.info(f"[OK] Manifiesto: {archivo_manifiesto}")
...
def generar_reporte_final(archivo):
    ...
    if archivo:
        ...
        logger.info(f"[OK] ÉXITO: Datos consolidados en {archivo.name}\n")
        ...
        logger.warning("[WARNING]  No se guardaron datos")
...
if __name__ == "__main__":
    logger.info("\n[LAUNCH] RECUPERADOR DE DATOS HISTÓRICOS - FASE 2 (LOCAL)\n")
