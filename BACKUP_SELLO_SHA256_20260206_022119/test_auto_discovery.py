...
class MockBus:
    ...
    def publicar(self, key: str, value: Any, unit: str = None):
        ...
        if self.count <= 20 or self.count % 100 == 0:
            logger.info(f"[STATS] Publicado #{self.count}: {key} = {value} {unit if unit else ''}")
...
async def test_auto_discovery():
    ...
    try:
        ...
        logger.info("[OK] BusExpander creado correctamente")
    except Exception as e:
        logger.error(f"[ERROR] Error creando BusExpander: {e}")
        return
    
    # Ejecutar solo el auto-discovery
    logger.info("\n" + "=" * 80)
    logger.info("[BUSCAR] Ejecutando AUTO-DISCOVERY (Sección 41)...")
    logger.info("=" * 80 + "\n")
    ...
    try:
        ...
        logger.info("\n" + "=" * 80)
        logger.info(f"[OK] AUTO-DISCOVERY COMPLETADO!")
        logger.info("=" * 80)
        logger.info(f"[STATS] Subfactores descubiertos automáticamente: {discovered}")
        logger.info(f"📦 Total acumulado en Bus: {count_after}")
        
        # Mostrar ejemplos de valores descubiertos
        logger.info("\n" + "=" * 80)
        logger.info("[BUSCAR] MUESTRA DE VALORES AUTO-DESCUBIERTOS (primeros 30):")
        logger.info("=" * 80)
        ...
        if discovered < 50:
            logger.warning(f"[WARNING]  ADVERTENCIA: Solo se encontraron {discovered} subfactores.")
            ...
            logger.warning(f"[WARNING]  Se encontraron {discovered} subfactores (menos de lo esperado 500-1500)")
            logger.warning("    Es posible que algunos motores no pudieran instanciarse.")
        else:
            logger.info(f"[OK] Cantidad de subfactores dentro del rango esperado (500-1500)")
        
        return discovered
        
    except Exception as e:
        logger.error(f"[ERROR] Error ejecutando auto-discovery: {e}")
        import traceback
        traceback.print_exc()
        return 0
...
async def test_full_expansion():
    ...
    try:
        ...
        logger.info("\n" + "=" * 80)
        logger.info("[OK] EXPANSIÓN COMPLETA TERMINADA")
        logger.info("=" * 80)
        logger.info(f"⏱️  Tiempo de ejecución: {duration:.2f} segundos")
        logger.info(f"[STATS] Total constantes publicadas: {bus.count}")
        logger.info(f"[TARGET] Promedio: {bus.count / duration:.0f} constantes/segundo")
        logger.info("=" * 80)
        ...
        if bus.count >= 2000:
            ...
            logger.warning(f"[WARNING]  Aún no se alcanza la meta de 2000 (actual: {bus.count})")
        
        return bus.count
        
    except Exception as e:
        logger.error(f"[ERROR] Error en test completo: {e}")
        import traceback
        traceback.print_exc()
        return 0
