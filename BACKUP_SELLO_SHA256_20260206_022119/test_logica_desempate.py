...
def test_desempate():
    ...
    total = len(tests)
    print(f"\n[OK] Nueva lógica: {correctos_new}/{total} correctos ({correctos_new*100//total}%)")
    
    if incorrectos_new == 0:
        print(f"\n[OK] PERFECTO - Lógica de desempate funciona correctamente")
    else:
        print(f"\n[WARNING]  REVISAR - Hay {incorrectos_new} casos fallidos")
