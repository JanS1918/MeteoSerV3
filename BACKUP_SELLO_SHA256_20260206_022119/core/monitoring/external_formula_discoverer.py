...
class ExternalFormulaDiscoverer:
    ...
    def discover_for_parameter(
        ...
        logger.info(f"[BUSCAR] Descubiertas {len(candidatas)} candidatas para {parametro}")
        ...
        for cand in candidatas:
            ...
            if validacion.get("valida"):
                ...
                logger.info(f"  [OK] {cand.nombre} validada (score: {cand.score_validacion:.2f})")
            else:
                result.detalles.append({
                    "id": cand.id,
                    "nombre": cand.nombre,
                ...
                logger.info(f"  [ERROR] {cand.nombre}: {validacion.get('razon', 'rechazada')}")
    ...
    def _descubrir_desde_scipy(
        ...
        for i, spec in enumerate(candidatas_para_param):
            ...
            if BLOCKER_DISPONIBLE and should_skip_candidate(spec):
                logger.warning(f"[WARNING] BLOQUEADA (V36.2): {spec.get('ref')} - {spec.get('nombre')}")
                continue
    ...
    def marcar_ganadora(self, id: str) -> bool:
        ...
        self._save_candidates()
        logger.info(f"[OK] {id} marcada como ganadora para duelo")
        return True
