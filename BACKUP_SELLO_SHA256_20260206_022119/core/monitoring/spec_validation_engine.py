...
class SpecValidationEngine:
    ...
    def validate_all_formulas(self, formula_hierarchy: Dict, implementations: Dict) -> Dict[str, ValidationResult]:
        ...
        """
        print("\n" + "="*80)
        print("[BUSCAR] VALIDACIÓN PROACTIVA DE ESPECIFICACIONES")
        print("="*80)
    ...
    def _validate_single_formula(self, formula_name: str, spec, implementations: Dict) -> ValidationResult:
        ...
        if formula_name not in implementations:
            errors.append(f"[ERROR] No existe implementación")
        ...
        try:
            ...
            errors.append(f"[ERROR] No se puede inspeccionar: {e}")
            return ValidationResult(
                formula_name=formula_name,
                is_valid=False,
                errors=errors,
        ...
        if non_canonical_spec:
            warnings.append(
                f"[WARNING] Parámetros no canónicos en spec: {non_canonical_spec}"
            )
        if non_canonical_impl:
            warnings.append(
                f"[WARNING] Parámetros no canónicos en implementación: {non_canonical_impl}"
            )
        ...
        if missing_params:
            errors.append(f"[ERROR] Parámetros requeridos faltantes: {missing_params}")
        
        if extra_params:
            warnings.append(f"[WARNING] Parámetros adicionales no en spec: {extra_params}")
        ...
        if not doc:
            warnings.append(f"[WARNING] Sin documentación")
    ...
    def _validate_vapor_pressure(self, name: str, func: Callable, spec, errors: List, warnings: List):
        ...
        if "real" in doc or "vapor actual" in doc:
            ...
            if "enhancement" not in doc and "factor" not in doc:
                warnings.append(
                    f"[WARNING] Calcula presión vapor 'real' pero no menciona Enhancement Factor"
                )
        ...
        if "iapws" in name.lower():
            ...
            if "humedad" not in params_norm:
                if "saturacion" not in name.lower():  # Si no es solo saturación
                    warnings.append(
                        f"[WARNING] IAPWS sin parámetro de humedad - verificar si está incompleta"
                    )
    ...
    def _validate_density(self, name: str, func: Callable, spec, errors: List, warnings: List):
        ...
        if "omm" in name.lower() or "virtual" in name.lower():
            if "virtual" not in doc:
                warnings.append(f"[WARNING] Densidad OMM sin mención de temperatura virtual")
    ...
    def _validate_utci(self, name: str, func: Callable, spec, errors: List, warnings: List):
        ...
        if "radiacion" not in params_norm:
            warnings.append(f"[WARNING] UTCI sin parámetro de radiación - verificar")
    ...
    def generate_report(self) -> Tuple[int, int]:
        ...
        print("\n" + "="*80)
        print("[STATS] REPORTE DE VALIDACIÓN")
        print("="*80)
        ...
        print(f"\n[OK] Válidas:    {valid}/{total}")
        print(f"[ERROR] Inválidas:   {invalid}/{total}")
        print(f"[WARNING] Warnings:   {len(self.warnings)}")
        
        if self.fatal_errors:
            print(f"\n[CRITICAL] ERRORES FATALES ({len(self.fatal_errors)}):")
...
def validate_spec_on_startup(formula_hierarchy: Dict, implementations: Dict):
    ...
    if fatal_count > 0:
        logger.critical(f"[CRITICAL] {fatal_count} errores fatales de especificación detectados")
