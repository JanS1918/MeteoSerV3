...
class BusExpander:
    ...
    async def _publish_vapor(self):
        ...
        try:
            ...
            # ✅ HARDY NIST: Wexler-Hyland (±0.05°C vs Magnus ±0.35°C)
            e_sat = calcular_presion_vapor_saturado_wexler(temp_c)
            ...
            e_actual = calcular_presion_vapor_real_hardy(temp_c, humedad, presion_pa)
            ...
            try:
                ...
                td = calcular_temperatura_rocio_hardy(temp_c, humedad, presion_pa)
                ...
                relacion_mezcla = calcular_relacion_mezcla(temp_c, humedad, presion_pa)
    ...
    async def _publish_trinity_elite(self):
        ...
        try:
            ...
            logger.info(f"   [STATS] Hardy: {len([k for k in self.bus._dict.keys() if k.startswith('hardy_')])} parámetros")
            logger.info(f"   [STATS] OMM: {len([k for k in self.bus._dict.keys() if k.startswith('omm_')])} parámetros")
            logger.info(f"   [STATS] REST2: {len([k for k in self.bus._dict.keys() if k.startswith('rest2_')])} parámetros")
            logger.info(f"   [STATS] Trinity: {len([k for k in self.bus._dict.keys() if k.startswith('trinity_')])} parámetros")
            logger.info(f"   [GUARDIAN] Salud sistema Trinity: {self.bus.leer('trinity_salud_sistema_score'):.1f}/100")
    ...
    async def _publish_astronomia(self):
        ...
        try:
            ...
            try:
                ...
                self.bus.publicar("arco_solar", elevacion_solar_spa_nrel, "grados")  # Alias para compatibilidad
        ...
        try:
            ...
            try:
                ...
                if temp_c < -15 or humedad > 90:
                    ...
                    utci_result_extremo = utci_v2_blazejczyk(temp_c, humedad, viento, radiacion)
                    self.bus.publicar("utci_extremos_v2", utci_result_extremo, "°C")
                else:
    ...
    async def _publish_cetreria(self):
        ...
        try:
            ...
            logger.info(f"[OK] Cetrería publicada: ST={st_cetrera:.1f}°C")
        ...
        try:
            ...
            try:
                ...
                if isinstance(loc, dict):
                    altitud = float(loc.get("altitud", 0.0) or 0.0)
                elif loc is not None and hasattr(loc, "altitud"):
                    altitud = float(getattr(loc, "altitud", 0.0) or 0.0)
            except Exception:
                altitud = 0.0
            ...
            cape_jkg = float(cape_result.get("cape_jkg", 0.0))
            ...
            lluvia_rate = self.system.data.get("lluvia_rate")
            ...
            prob_tormenta = max(0.0, min(100.0, float(sev.get("riesgo_tormenta_severa_pct", 0.0))))
            ...
            self.bus.publicar("confianza_prediccion_lluvia", min(100, 30 + prob_lluvia * 0.7), "%")
            ...
            deard = get_temperatura_minima_v47_0(
                ...
                radiacion_neta_wm2=qnet_val if qnet_val is not None else -70.0,
                viento_ms=viento_ms,
                humedad_relativa=humedad,
                tipo_suelo=tipo_suelo,
                horas_hasta_amanecer=8.0,
                lluvia_ultimas_24h_mm=lluvia_24h,
                usar_prata_t_cielo=True,  # V47.0: Activar Prata (1996)
            )
    ...
    async def _publish_calidad_aire_visibilidad(self):
        ...
        try:
            ...
            # VISIBILIDAD STOELINGA-WARNER (microfísica Thompson)
            ...
            indice_visibilidad = max(0.0, min(100.0, (visibilidad_m / 50000.0) * 100.0))
            ...
            logger.info(f"[OK] Calidad aire publicada (AQI:{aqi_compuesto:.0f}, Visibilidad:{visibilidad_km:.1f}km, CO2:{co2:.0f}ppm)")
    ...
    async def _publish_confort_avanzado(self):
        ...
        try:
            ...
            try:
                ...
                if contexto is None:
                    ...
                    class _Ctx:
                        ...
                        presion_barometrica = (self.system.data.get("presion_barometrica", 101325.0) / 100.0)
                        elevacion_solar = 45.0
                        hora_utc = None
                    contexto = _Ctx()
            ...
            try:
                ...
                self.bus.publicar("cin", cape_result.get("cin_jkg", 0.0), "J/kg")
                self.bus.publicar("lfc", cape_result.get("lfc_m", 0.0), "m")
                self.bus.publicar("el", cape_result.get("el_m", 0.0), "m")
                ...
                self.bus.publicar("cin", None, "J/kg")
                self.bus.publicar("lfc", None, "m")
                self.bus.publicar("el", None, "m")
                self.bus.publicar("lcl", None, "m")
                self.bus.publicar("cape_categoria", "Indeterminado", "string")
            ...
            try:
                ...
                self.bus.publicar("lifted_index_categoria", "Indeterminado", "string")
        ...
        try:
            ...
            try:
                ...
                et0 = evapotranspiracion_penman_monteith(
                    temp_c, humedad_aire, radiacion, viento,
                    altitud=self.system.ubicacion.get('altitud', 200.0)
                )
    ...
    async def _publish_cetreria(self):
        ...
        try:
            ...
            logger.info(f"[OK] Cetrería bonus publicada: ST={st_cetrera:.1f}°C")
    ...
    async def _publish_uv_aerosoles_dinamicos(self):
        ...
        try:
            ...
            logger.info(f"[OK] UV/Aerosoles (AOD500:{aod_500nm:.3f}, Ozono:{ozono_base:.0f}DU, Kt:{Kt:.2f})")
    ...
    async def _publish_modelos_avanzados_ocultos(self):
        ...
        try:
            ...
            presion_hpa = self.system.sensores.get("presion_barometrica", 101325.0) / 100.0 if hasattr(self.system, "sensores") else self.system.data.get("presion_barometrica", 101325.0) / 100.0
            viento = self.system.data.get("viento", 0.0)
            radiacion = self.system.data.get("radiacion", 0.0)
    ...
    async def _publish_modelos_especializados_finales(self):
        ...
        try:
            ...
            try:
                ...
                self.bus.publicar("wbgt_tg", Tg, "°C")
