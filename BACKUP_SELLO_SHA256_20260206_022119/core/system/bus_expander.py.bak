...
class BusExpander:
    ...
    async def _publish_precision_calibracion(self):
        ...
        try:
            ...
            # ESTADO CALIBRACIÓN
            ultima_calibracion = getattr(self.system, '_ultima_calibracion', datetime.now())
    ...
    async def _publish_elite_motors_v25(self):
        ...
        6. Simulación Forense - integridad SHA256
        """
        try:
            ...
            temp_c = self.system.data.get("temperatura", 15.0)
            presion_hpa = self.system.data.get("presion_barometrica", 1013.25) / 100.0 if self.system.data.get("presion_barometrica", 0) > 10000 else self.system.data.get("presion_barometrica", 1013.25)
            humedad = self.system.data.get("humedad", 50.0)
            ...
            t_ground, info_capa = motor_capa.calcular_t_ground(
                ...
                estabilidad_monin=0.0  # Placeholder
            )
            ...
            self.bus.publicar("altura_capa_limite_estimada", 1000.0, "m")  # Placeholder
            ...
            # Radiación teórica simplificada
            lat = self.system.location.get("latitud", 41.5) if hasattr(self.system, 'location') else 41.5
            day_of_year = datetime.now().timetuple().tm_yday
            angulo_cenital = 45.0  # Placeholder
            rad_teorica = 1000.0 * math.cos(math.radians(angulo_cenital))
            ...
            # Estimar delta_p desde viento
            densidad_aire = 1.225  # kg/m³ típico
            delta_p = 0.5 * densidad_aire * (viento_kmh / 3.6) ** 2  # Bernoulli
            ...
            punto_rocio = temp_c - ((100 - humedad) / 5.0)  # Estimación rápida
    ...
    async def _publish_funciones_auxiliares_fisica(self):
        ...
        try:
            ...
            # Altitud actual
            altitud = self.system.location.get("altitud", 100.0) if hasattr(self.system, 'location') else 100.0
            ...
            if presion_hpa is not None and presion_hpa > 900 and presion_status == "OK":
                try:
                    mo_stab = monin_obukhov_stability(
                        ...
                        latitud=self.system.data.get("latitud", 41.5513)
                    )
                    ...
                    self.bus.publicar("monin_obukhov_g", 9.81, "m/s²")
    ...
    async def _publish_environmental_indices_auxiliares(self):
        ...
        try:
            ...
            self.bus.publicar("gravedad_estandar", 9.80665, "m/s²")
