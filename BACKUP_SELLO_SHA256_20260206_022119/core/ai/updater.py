...
class UpdaterEngine:
    ...
    def __init__(self, updates_dir: str = "data/updates", backup_dir: str = "backups"):
        ...
        logger.info(f"[REINICIO] UpdaterEngine inicializado")
    ...
    def _load_history(self) -> None:
        ...
        if history_file.exists():
            ...
            logger.info(f"[OK] Historial de updates cargado: {len(self.update_history)} entradas")
    ...
    def check_for_updates(self, repo_url: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"[OK] Encontradas {len(updates)} actualizaciones disponibles")
                return updates
            else:
                logger.error(f"[ERROR] Error consultando updates: {response.status_code}")
                return []
        except Exception as e:
            logger.error(f"[ERROR] Error verificando updates: {e}")
            return []
    ...
    def download_update(self, update_url: str, update_id: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            logger.info(f"[OK] Update descargado: {update_file}")
        ...
        try:
            ...
            if expected_checksum:
                if checksum == expected_checksum:
                    logger.info(f"[OK] Checksum válido: {checksum[:16]}...")
                ...
                logger.info(f"[INFO] Checksum calculado: {checksum[:16]}... (sin validación)")
                return True, checksum
    ...
    def create_backup(self, backup_name: Optional[str] = None) -> Tuple[bool, Optional[str]]:
        ...
        try:
            logger.info(f"[GUARDAR] Creando backup: {backup_name}")
    ...
    def apply_update(self, update_file: str, dry_run: bool = False) -> Tuple[bool, str]:
        ...
        try:
            ...
            logger.info(f"[REINICIO] Aplicando update: {update_file} (dry_run={dry_run})")
            
            if dry_run:
                logger.info("[INFO] Modo dry-run: no se aplicarán cambios reales")
            ...
            logger.info(f"[OK] Update aplicado exitosamente")
    ...
    def rollback(self, backup_path: Optional[str] = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            logger.info(f"[OK] Rollback completado")
    ...
    def update_pip_packages(self, packages: Optional[List[str]] = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info(f"[OK] Paquetes actualizados exitosamente")
