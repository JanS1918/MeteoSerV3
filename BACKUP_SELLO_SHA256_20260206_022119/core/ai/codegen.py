        ...
        if not self.api_key:
            ...
            try:
                ...
                try:
                    ...
                    send_telegram_message("[WARNING] IA en modo local: sin API key ni Llama local. Solo templates.")
    ...
    def _generate_with_llama_local(self, prompt: str, language: str, context: Optional[Dict]) -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"[OK] Código generado exitosamente (Llama local, {len(code)} chars)")
                return code, None
            else:
...
Responde SOLO con el código, sin explicaciones adicionales."""
            ...
            if response.status_code == 200:
                ...
                logger.info(f"[OK] Código generado exitosamente ({len(code)} chars)")
                return code, None
            else:
                ...
                # Fallback a template
                logger.info("[REINICIO] Fallback a template...")
                return self._generate_with_template(prompt, context)
...
'''
    ...
    def validate_syntax(self, code: str, language: str = "python") -> Tuple[bool, Optional[str]]:
        ...
        try:
            ast.parse(code)
            logger.info("[OK] Sintaxis válida")
            return True, None
    ...
    def run_tests(self, code_path: str, test_path: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if success:
                logger.info(f"[OK] Tests pasaron exitosamente")
            else:
                logger.error(f"[ERROR] Tests fallaron:\n{output}")
    ...
    def save_code(self, code: str, output_path: str) -> bool:
        ...
        try:
            ...
            logger.info(f"[GUARDAR] Código guardado en: {output_path}")
            return True
        except Exception as e:
            logger.error(f"[ERROR] Error guardando código: {e}")
            return False
