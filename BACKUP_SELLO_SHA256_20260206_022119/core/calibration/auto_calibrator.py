...
try:
    ...
    print("[WARNING] scikit-learn no disponible. Instalar con: pip install scikit-learn")
...
try:
    ...
    print("[WARNING] Módulos avanzados (bias_detector, regression_calibrator) no disponibles")
...
class AutoCalibrador:
    ...
    def entrenar_desde_bus(self, sensor: str, dias: int = 7) -> ModeloCalibracion:
        ...
        print(f"\n[STATS] Entrenando calibración para '{sensor}'")
        ...
        if desvio < 0.1:
            print(f"   [WARNING]  Sensor muy estable (σ={desvio:.3f}), posible sensor pegado")
            return None
        ...
        if SKLEARN_AVAILABLE:
            ...
            if abs(deriva_por_dia) > 0.05:
                ...
                print(f"   [OK] Corrección LINEAL (deriva detectada)")
            else:
                ...
                print(f"   [OK] Corrección OFFSET ({intercept:+.3f})")
            ...
            print("   [ERROR] scikit-learn no disponible, no se puede entrenar")
            return None
    ...
    def guardar_modelo(self, sensor: str):
        ...
        print(f"   [GUARDAR] Modelo guardado: {ruta}")
    ...
    def cargar_modelo(self, sensor: str) -> bool:
        ...
        print(f"   [OK] Modelo cargado: {sensor} ({self.modelos[sensor].tipo})")
        return True
...
class OrquestadorCalibracion:
    ...
    def entrenar_desde_referencias(self, sensor: str, 
        ...
        if not MODULOS_AVANZADOS:
            print("[WARNING] Módulos avanzados no disponibles - usando calibración básica")
            return {}
        ...
        print(f"\n[STATS] REPORTE DE DETECCIÓN - {sensor}:")
        ...
        if metricas['mae'] < 0.3:  # MAE < 0.3 es excelente
            ...
            print(f"[OK] Calibrador '{sensor}' ACTIVADO (MAE={metricas['mae']:.3f} < 0.3)")
        else:
            print(f"[WARNING]  Calibrador '{sensor}' entrenado pero NO activado (MAE={metricas['mae']:.3f} > 0.3)")
    ...
    def iniciar_deteccion_automatica(self, intervalo_horas: int = 6):
        ...
        """
        if not MODULOS_AVANZADOS:
            print("[WARNING] Módulos avanzados no disponibles")
            return
            ...
            try:
                ...
                for sensor in self.detector.historicos.keys():
                    ...
                    if "URGENTE" in reporte['conclusion']:
                        print(f"\n[CRITICAL] ALERTA: Sensor '{sensor}' requiere calibración")
    ...
    def guardar_todo(self, directorio: str = "data/calibration"):
        ...
        print(f"[GUARDAR] Todos los calibradores guardados en {directorio}")
