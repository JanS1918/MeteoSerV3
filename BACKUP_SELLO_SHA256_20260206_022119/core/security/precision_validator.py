...
class PrecisionValidator:
    ...
    def validate_measurement(self,
        ...
        if required_precision is None:
            ...
            return False, f"[WARNING] Parámetro '{parameter_name}' no tiene precisión definida"
        ...
        if error <= required_precision:
            ...
            return True, f"[OK] PRECISIÓN OK: {parameter_name} error {error:.4f} <= {required_precision}"
        else:
            self.validation_log.append({
                "parameter": parameter_name,
                "measured": measured_value,
            ...
            return False, f"[ERROR] PRECISIÓN INSUFICIENTE: {parameter_name} error {error:.4f} > {required_precision}"
    ...
    def validate_formula_output(self,
        ...
        if min_output <= measured_output <= max_output:
            return True, f"[OK] RANGO OK: {parameter_name} = {measured_output} ∈ [{min_output}, {max_output}]"
        else:
            return False, f"[ERROR] FUERA DE RANGO: {parameter_name} = {measured_output} ∉ [{min_output}, {max_output}]"
