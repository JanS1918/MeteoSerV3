"""
═══════════════════════════════════════════════════════════════════════════════
[GUARDIAN] MOS V47.2 - CLUSTERING INTELIGENTE CON VALIDACIÓN ESTACIONAL
═══════════════════════════════════════════════════════════════════════════════
...
class MOSClusteringV472:
    ...
    def __init__(self, bus=None, data_path: str = "data"):
        ...
        logger.info("[GUARDIAN] MOS V47.2 Clustering Inteligente inicializado")
    ...
    def validar_predicciones_pendientes(self, obtener_valor_real_callback):
        ...
        for parametro in self.parametros_mos:
            ...
            for pred in predicciones:
                ...
                logger.info(
                    f"[OK] {parametro} validado: predicho={pred['valor_predicho']:.2f}, "
    ...
    def _registrar_evento_en_escenario(self, parametro: str, escenario: str, evento: Dict):
        ...
        if similitud < UMBRAL_SIMILITUD:
            logger.warning(
                f"[WARNING] {parametro} escenario {escenario}: Evento con baja similitud "
        ...
        if escenarios[escenario]["eventos"]:
            ...
            if abs(evento["bias"] - bias_promedio) > UMBRAL_ANOMALIA:
                logger.warning(
                    f"[WARNING] {parametro} escenario {escenario}: Evento anómalo detectado "
    ...
    def _activar_correccion_escenario(self, parametro: str, escenario: str):
        ...
        logger.critical(
            f"[GUARDIAN] MOS {parametro} escenario {escenario} ACTIVADO: "
    ...
    def _registrar_epoca_actual(self, parametro: str, escenario: str):
        ...
        logger.info(
            f"[FECHA] {parametro} escenario {escenario}: Nueva época {epoca_actual} "
    ...
    def _validar_estacionalidad_completa(self, parametro: str, escenario: str):
        ...
        if std_dev_anual < 0.02:  # Umbral estricto
            ...
            logger.info(
                f"[INFO] {parametro} escenario {escenario}: BIAS varía estacionalmente "
    ...
    def auditoria_global_consistencia(self, parametro: str):
        ...
        if std_dev_global < UMBRAL_CONSISTENCIA_GLOBAL:
            # [CRITICAL] SESGO SISTEMÁTICO DETECTADO
            if self.bus:
            ...
            logger.critical(
                f"[CRITICAL] {parametro}: SESGO SISTEMÁTICO DETECTADO. "
            ...
            logger.info(
                f"[OK] {parametro}: Física diferenciada por escenario "
    ...
    def monitorear_drift_bias(self, parametro: str, escenario: str):
        ...
        if delta > UMBRAL_DRIFT_BIAS:
            # [CRITICAL] BIAS HA CAMBIADO
            logger.warning(
                f"[WARNING] {parametro} escenario {escenario}: BIAS cambió de "
    ...
    def _reiniciar_aprendizaje_escenario(self, parametro: str, escenario: str):
        ...
        logger.info(
            f"[REINICIO] {parametro} escenario {escenario}: Re-aprendizaje iniciado "
    ...
    def _cargar_estado(self):
        ...
        try:
            ...
            logger.info(f"[OK] Estado MOS V47.2 cargado: {len(self.parametros_mos)} parámetros")
        except Exception as e:
