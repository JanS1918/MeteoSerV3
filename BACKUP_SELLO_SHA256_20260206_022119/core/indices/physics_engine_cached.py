...
class PhysicsEngineCached:
    ...
    def obtener_todas_constantes(self, altitud_m: float = 0.0) -> Dict:
        ...
        if cached is not None:
            logger.debug(f"[TARGET] Cache HIT para {clave}")
            return cached
        
        # Calcular y almacenar
        logger.debug(f"[ERROR] Cache MISS para {clave}, calculando...")
    ...
    def actualizar_parametros(self, temperatura_k: float = None, 
        ...
        if cambio:
            logger.debug(f"[WARNING] Parámetros actualizados, limpiando caché")
            self.cache.clear()
