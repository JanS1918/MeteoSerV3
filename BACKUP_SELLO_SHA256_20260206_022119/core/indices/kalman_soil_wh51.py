...
def aplicar_kalman_soil_wh51(
    humedad_actual: float,
    temperatura_c: Optional[float] = None,
    lluvia_24h_mm: Optional[float] = None,
    usar_cold_start: bool = False,
    Q_humedad: float = 0.01,
    Q_tendencia: float = 0.001,
    R_medicion: float = 0.1
) -> Dict[str, float]:
    """
    Aplica Kalman WH51 en modo singleton para uso en runtime.

    Args:
        humedad_actual: Medición actual (%)
        temperatura_c: Temperatura (no usado, reservado)
        lluvia_24h_mm: Lluvia acumulada (no usado, reservado)
        usar_cold_start: Reinicia filtro con estado actual
        Q_humedad: Ruido proceso humedad
        Q_tendencia: Ruido proceso tendencia
        R_medicion: Ruido medición sensor

    Returns:
        Dict con humedad_filtrada, tendencia, ganancia_kalman, innovacion, incertidumbre
    """
    global _KALMAN_INSTANCE

    if _KALMAN_INSTANCE is None or usar_cold_start:
        estado_inicial = np.array([max(0.0, min(100.0, humedad_actual)), 0.0])
        _KALMAN_INSTANCE = KalmanSoilWH51(
            Q_humedad=Q_humedad,
            Q_tendencia=Q_tendencia,
            R_medicion=R_medicion,
            estado_inicial=estado_inicial
        )

    return _KALMAN_INSTANCE.procesar(humedad_actual)
...
if __name__ == "__main__":
    ...
    print("[STATS] RESULTADOS FILTRADO:")
    ...
    print("[STATS] COMPARACIÓN MEDICIÓN vs FILTRADO:")
    ...
    print("\n[OK] TEST COMPLETADO - Kalman Soil WH51 funcional")
