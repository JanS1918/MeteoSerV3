...
def main():
    ...
    print(f"[OK] {len(duelos_ejecutados) if duelos_ejecutados else 0} duelos ejecutados")
    ...
    if duelos_path.exists():
        ...
        print(f"[OK] {len(duelos)} duelos en histórico")
        ...
        for i, duelo in enumerate(duelos, 1):
            ...
            if formula_a_resultado and formula_b_resultado:
                if formula_a_resultado > formula_b_resultado:
                    print(f"    [OK] CORRECTO - A > B ({formula_a_resultado:.2f} > {formula_b_resultado:.2f})")
                else:
                    print(f"    [ERROR] ERROR - A <= B ({formula_a_resultado:.2f} <= {formula_b_resultado:.2f})")
                    errores += 1
        
        if errores == 0:
            print(f"\n[OK] TODOS LOS DUELOS SON CORRECTOS - Histórico integro")
        else:
            print(f"\n[ERROR] {errores} ERRORES ENCONTRADOS")
    else:
        print("[WARNING]  No hay histórico de duelos")
