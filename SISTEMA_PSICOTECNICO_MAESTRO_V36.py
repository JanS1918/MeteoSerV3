            ...
            try:
                ...
                if not test_result.passed:
                    if capa_def.type == CapaType.HARD:
                        logger.error(f"  [ERROR] CAPA DURA FALLÓ - Rechazando candidata")
                        journey.failed_capas.append(capa_id)
                        break  # Detiene aquí
                    else:
                        logger.warning(f"  [WARNING]  Capa flexible falló, continúa")
                        journey.failed_capas.append(capa_id)
                else:
                    logger.info(f"  [OK] Pasó")
                ...
                logger.error(f"  [ERROR] Error en capa {capa_id}: {e}")
                journey.failed_capas.append(capa_id)
...
class MLCalibrator:
    ...
    def add_historical_records(self, records: List[HistoricalRecord]) -> None:
        ...
        logger.info(f"[OK] Cargados {len(clean_records)} registros históricos LIMPIOS")
    ...
    async def calibrate_weights(self) -> Tuple[float, float, float]:
        ...
        if len(self.historical_records) < 20:
            logger.warning("[WARNING]  Insuficientes datos para calibrar (min 20)")
        ...
        logger.info(f"[OK] Pesos calibrados: w1={best_weights[0]:.2f}, "
    ...
    async def execute_robust_duel(
        ...
        if result.candidate_score > result.current_score:
            ...
            if is_sig and result.margin >= 0.10:
                ...
                logger.info(f"[OK] ACEPTADA - Gana en TODAS (p={p_val:.4f}, margin={result.margin:.2%})")
            elif result.margin >= 0.05:
                result.decision = DuelDecision.NEEDS_REVIEW
                logger.warning(f"[WARNING]  REVISAR - Margen estrecho (margin={result.margin:.2%})")
            else:
                result.decision = DuelDecision.REJECTED
                logger.info(f"[ERROR] RECHAZADA - Margen muy bajo ({result.margin:.2%})")
        else:
            result.outcome = DuelOutcome.ACTUAL_WINS
            result.decision = DuelDecision.REJECTED
            logger.info(f"[ERROR] RECHAZADA - Actual es mejor")
...
class Watchdog24h:
    ...
    async def monitor_deployed_candidate(
        ...
        if change_percent < -0.10:
            ...
            logger.error(f"[CRITICAL] ROLLBACK AUTOMÁTICO: {candidate_id} degradó {change_percent:.1%}")
            ...
            self.alerts.append(alert)
            logger.warning(f"[WARNING]  {candidate_id} degradó {change_percent:.1%}")
            return alert
...
class PsychotechnicOrchestrator:
    ...
    async def validate_and_duel(
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"[LAUNCH] INICIO VALIDACIÓN COMPLETA: {candidate_id}")
        logger.info(f"{'='*80}\n")
        ...
        if not journey.passed_all_capas:
            logger.error(f"[ERROR] No pasó psicotécnico - Rechazada")
            return DuelResult(
                candidate_id=candidate_id,
                current_id=current_id,
        ...
        if journey.justice_score < 0.75:
            logger.error(f"[ERROR] Justice Score insuficiente: {journey.justice_score:.2%}")
            return DuelResult(
                candidate_id=candidate_id,
                current_id=current_id,
                decision=DuelDecision.REJECTED
        ...
        logger.info(f"[OK] Psicotécnico PASADO - justice_score={journey.justice_score:.2%}\n")
        ...
        if risk_score > 0.7:
            logger.error(f"[ERROR] Riesgo cascada ALTO: {risk_score:.2%} - NOTIFICAR_HUMANO")
            return DuelResult(
                candidate_id=candidate_id,
                current_id=current_id,
        ...
        logger.info(f"[OK] Cascade OK - risk={risk_score:.2%}\n")
        ...
        logger.info(f"[OK] Duelo completado - decision={duel_result.decision.value}\n")
        ...
        if duel_result.decision == DuelDecision.ACCEPTED:
            ...
            logger.info(f"[OK] Watchdog configurado\n")
...
class CalculationSpecificityAnalyzer:
    ...
    async def analyze_formula(
        ...
        logger.info(f"[STATS] {formula_id} afecta {len(affected)} cálculos")
        ...
        if profile.better_in and len(profile.worse_in) <= 2:
            ...
            logger.info(f"[OK] Aceptar SOLO para: {[c.value for c in profile.better_in]}")
        else:
            profile.overall_decision = "REJECT"
            logger.info(f"[ERROR] Rechazar - No es mejor para cálculos específicos")
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("\n[OK] 25 Capas inicializadas")
    print("[OK] 5 Mitigaciones integradas")
    print("[OK] Mejoras V36 activadas")
    print("[OK] Detector de Especificidad (cálculos específicos)")
    print("[OK] Aplicable a FÓRMULAS y ALGORITMOS")
