...
def pytest_pyfunc_call(pyfuncitem):
    ...
    if inspect.iscoroutinefunction(testfunction):
        ...
        if any(p.kind == inspect.Parameter.VAR_KEYWORD for p in signature.parameters.values()):
            funcargs_filtered = funcargs
        else:
            funcargs_filtered = {k: v for k, v in funcargs.items() if k in signature.parameters}
        ...
        try:
            ...
            loop.run_until_complete(testfunction(**funcargs_filtered))
