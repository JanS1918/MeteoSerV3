...
def _run_discovery():
    ...
    logger.info("\n" + "=" * 70)
    logger.info("[BUSCAR] PRUEBA 1: DESCUBRIMIENTO DE CANDIDATAS EXTERNAS")
    logger.info("=" * 70)
    ...
    logger.info(f"\n[OK] Descubiertas {len(candidatas)} candidatas válidas:")
...
def test_duel():
    resultado = _run_duel()
    assert hasattr(resultado, "ganadora")
    assert hasattr(resultado, "score_ganadora")
...
def _run_integration():
    ...
    if success:
        logger.info("[OK] Integración exitosa")
        ...
        logger.info("[ERROR] Integración falló")

    return success


def test_integration():
    success = _run_integration()
...
def _run_orchestrator():
    ...
    logger.info("\n" + "=" * 70)
    logger.info("[TARGET] PRUEBA 4: ORQUESTADOR COMPLETO")
    logger.info("=" * 70)
...
def test_orchestrator():
    ciclo = _run_orchestrator()
    assert ciclo is not None


def main():
    logger.info("\n" + "=" * 70)
    logger.info("[LAUNCH] SISTEMA DE DESCUBRIMIENTO Y DUELO DE FÓRMULAS")
    logger.info("=" * 70)
    ...
    try:
        ...
        integracion = _run_integration()

        # Prueba 4: Orchestrator
        ciclo = _run_orchestrator()

        logger.info("\n" + "=" * 70)
        logger.info("[OK] TODAS LAS PRUEBAS COMPLETADAS EXITOSAMENTE")
        logger.info("=" * 70)
        logger.info(f"\nResumen:")
        ...
        logger.info("\n[REINICIO] Sistema listo para búsqueda automática periódica\n")

    except Exception as e:
        logger.exception(f"[ERROR] Error en pruebas: {e}")
        sys.exit(1)
