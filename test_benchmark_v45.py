...
async def test_latencia_bus():
    ...
    try:
        ...
        print("  [OK] Módulos vectorizados cargados")
    except ImportError as e:
        print(f"  [WARNING] Error cargando módulos: {e}")
        from core.indices.nubosidad_liu_jordan_kasten import _calcular_nubosidad_nocturna
        ...
        print("  [WARNING] Usando versión no vectorizada de Thompson")
    ...
    print(f"  [STATS] Nubosidad nocturna: {resultado_carmona.get('nubosidad', 'N/A')}%")
    print(f"  🔬 Emisividad cielo: {resultado_carmona.get('emisividad_cielo', 'N/A')}")
    print(f"  {'[ERROR] NaN DETECTADO' if nan_carmona else '[OK] SIN NaN'}")
    ...
    for _ in range(100):
        ...
        psi_m = np.empty_like(zeta_vec)
        mask_unstable = zeta_vec < 0
        if np.any(mask_unstable):
            x = 1.0 - 16.0 * zeta_vec[mask_unstable]
            x_quarter = np.power(x, 0.25)
            x_half = np.sqrt(x)
            psi_m[mask_unstable] = (
                2.0 * np.log((1.0 + x_quarter) / 2.0)
                + np.log((1.0 + x_half) / 2.0)
                - 2.0 * np.arctan(x_quarter)
                + np.pi / 2.0
            )
        psi_m[~mask_unstable] = -5.0 * zeta_vec[~mask_unstable]
    ...
    print(f"  [STATS] Perfil ref: {ln_vec[0]:.4f}, obj: {ln_vec[1]:.4f}")
    print(f"  🔬 Psi_m ref: {psi_m[0]:.4f}, obj: {psi_m[1]:.4f}")
    print(f"  {'[ERROR] NaN DETECTADO' if nan_gryning else '[OK] SIN NaN'}")
    ...
    print(f"  [STATS] qc (nube): {resultado_thompson.get('qc_gkg', 'N/A')} g/kg")
    print(f"  [STATS] qr (lluvia): {resultado_thompson.get('qr_gkg', 'N/A')} g/kg")
    print(f"  [STATS] Velocidad caída: {resultado_thompson.get('fall_speed_ms', 'N/A')} m/s")
    print(f"  {'[ERROR] NaN DETECTADO' if nan_thompson else '[OK] SIN NaN'}")
    
    # ═══════════════════════════════════════════════════════════
    # RESUMEN FINAL
    # ═══════════════════════════════════════════════════════════
    print("\n" + "=" * 80)
    print("[STATS] RESUMEN LATENCIA TRÍADA VECTORIZADA V45.0")
    print("=" * 80)
    ...
    print(f"  {'─' * 40}")
    print(f"  [FAST] TOTAL TRÍADA:      {tiempo_total:>8.4f} ms")
    print()
    print(f"  [TARGET] Throughput: {1000.0 / tiempo_total:.1f} ciclos/segundo")
    print(f"  {'─' * 40}")
    
    # Certificación estabilidad
    estable = not (nan_carmona or nan_gryning or nan_thompson)
    if estable:
        print(f"  [OK] CERTIFICACIÓN: CERO NaN DETECTADOS")
        print(f"  [OK] ESTABILIDAD: 100% GARANTIZADA")
    else:
        print(f"  [ERROR] ADVERTENCIA: NaN detectados en vectorización")
        if nan_carmona:
...
async def verificar_prediccion_lluvia():
    ...
    if logs_dir.exists():
        ...
        if archivos_pred:
            ...
            try:
                ...
                if lluvia_pred > 30:
                    print(f"  [OK] SÍ - MeteoSer predijo lluvia con {lluvia_pred}% probabilidad")
                else:
                    print(f"  [WARNING] Predicción baja: {lluvia_pred}%")
                    
            except Exception as e:
                print(f"  [WARNING] Error leyendo predicción: {e}")
        else:
            print(f"  [WARNING] No se encontraron archivos de predicción")
    ...
    try:
        ...
        if storm_likely:
            print(f"  [FAST] VENTANAS ROLLING DETECTARON PATRÓN DE TORMENTA")
        
    except Exception as e:
        print(f"  [WARNING] Bus no accesible: {e}")
...
async def main():
    ...
    print("\n[LAUNCH] METEOSER V45.0 - TEST COMPLETO DE VECTORIZACIÓN\n")
    ...
    print(f"\n{'[OK] TEST APROBADO' if resultados['estable'] else '[ERROR] TEST FALLIDO'}")
