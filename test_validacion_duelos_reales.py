...
def cargar_duelos_historicos():
    ...
    if not duelos_path.exists():
        print("[ERROR] No hay histórico de duelos (duelos_historico.jsonl)")
        return []
...
def cargar_datos_reales(ventana_horas=24):
    ...
    if not datos_path.exists():
        print("[ERROR] No hay histórico de sensores (sensores_historico.json)")
        return {}
...
def auditar_decision_duelo(duelo, validacion):
    ...
    if validacion['correcto'] is None:
        print(f"  [WARNING]  {validacion['razon']}")
    elif validacion['correcto']:
        print(f"  [OK] DECISIÓN CORRECTA - {validacion['razon']}")
    else:
        print(f"  [ERROR] DECISIÓN CUESTIONABLE - {validacion['razon']}")
...
def main():
    ...
    if not duelos:
        print("[ERROR] No hay duelos para validar")
        return
    
    print(f"[OK] {len(duelos)} duelos cargados")
    
    # 2. Cargar datos reales
    print("\n[2/4] Cargando datos sensoriales reales...")
    datos_reales = cargar_datos_reales()
    if not datos_reales:
        print("[WARNING]  Sin datos sensoriales para comparar")
    else:
        print(f"[OK] Datos sensoriales disponibles")
    ...
    print(f"  [OK] Decisiones correctas:  {resultados['correctas']} ({resultados['correctas']*100//resultados['total'] if resultados['total'] else 0}%)")
    print(f"  [ERROR] Decisiones cuestionables: {resultados['incorrectas']} ({resultados['incorrectas']*100//resultados['total'] if resultados['total'] else 0}%)")
    print(f"  [WARNING]  Indeterminadas:       {resultados['indeterminadas']}")
    print(f"  📈 Diferencia promedio:   {resultados['diferencia_promedio']:.4f}")
    
    # Diagnóstico
    print(f"\n[BUSCAR] DIAGNÓSTICO:")
    if resultados['correctas'] / resultados['total'] >= 0.9:
        print(f"  [OK] EXCELENTE - El motor de duelos toma decisiones confiables (>90%)")
    elif resultados['correctas'] / resultados['total'] >= 0.7:
        print(f"  [WARNING]  ACEPTABLE - Decisiones mayoritariamente correctas (70-90%)")
    elif resultados['correctas'] / resultados['total'] >= 0.5:
        print(f"  [WARNING]  CUESTIONABLE - Menos del 70% de decisiones son correctas")
    else:
        print(f"  [ERROR] PROBLEMA - Menos del 50% de decisiones son correctas")
    
    # Mostrar detalles de duelos problemáticos
    problemáticos = [d for d in resultados['detalles'] if d['validacion']['correcto'] is False]
    if problemáticos:
        print(f"\n[WARNING]  DUELOS CUESTIONABLES ({len(problemáticos)}):")
...
[OK] VALIDADO: {resultados['correctas']*100//resultados['total']}% confiabilidad
...
""", end="")
    ...
    if resultados['correctas'] / resultados['total'] >= 0.85:
        ...
        print("[WARNING]  El método necesita ajustes antes de usar en producción.")
