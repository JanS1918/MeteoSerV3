...
#
# 2. REDONDEO: ├ìndices meteorol├│gicos m├íximo 2 decimales en salida
#
# 3. CONFIG SOBERANA: Todos los l├¡mites desde data/indices_config.json
#
# 4. ESCUDO DE SEGURIDAD: Protecciones (if > 0) intactas en divisiones
#
# Ver: ENGINEERING_STANDARDS.md (ra├¡z del proyecto)
# ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ
...
def _validate_sensor_payload(normalized: dict) -> tuple[bool, list[str]]:
    ...
    if normalized.get("value") is None:
        errors.append("Valor no convertible a n├║mero")
    now = time.time()
...
async def feedback_prediccion(payload: dict = Body(...)):
    ...
    if nombre:
        ...
        # Si hay valor real y valor estimado num├®rico, registrar error cuantitativo
        try:
            if feedback == "error" and valor_real is not None and valor is not None:
                ...
                # Entrenamiento online si es predicci├│n
                if tipo == "prediccion":
...
if 'app' not in globals():
    app = FastAPI()
# Habilitar CORS para todos los or├¡genes (localhost, 127.0.0.1, etc.)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# Montar archivos est├íticos con ruta absoluta
if STATIC_DIR.exists():
    app.mount("/static", StaticFiles(directory=str(STATIC_DIR)), name="static")
else:
    print(f"WARNING: Static directory not found at {STATIC_DIR}")

# Inicializar sistema MeteoSer y forzar motores, con protecci├│n ante errores
try:
    manager = SystemManager()
    system = manager.iniciar()
    # Asegurar que el Cerebro Estad├¡stico est├® presente y trate de restaurar estado
    try:
        ...
        if not hasattr(system, 'statistical_brain') or system.statistical_brain is None:
            ...
            logger.info("­ƒºá StatisticalBrain a├▒adido al sistema (restore_state=True)")
    except Exception:
        # No bloquear el arranque si falla la creaci├│n del cerebro
        logger.exception("ÔÜá´©Å No se pudo inicializar StatisticalBrain en el arranque")
    if not isinstance(system.indices, EnvironmentalIndices):
        system.indices = EnvironmentalIndices(system)
    
    # --- LIMPIEZA DE BUFFERS DE ERROR: Monin-Obukhov y presi├│n ---
    import logging
    # Limpiar flags de error de presi├│n
        ...
        for meta in system.sensores_metadata.values():
            for k in list(meta.keys()):
                if 'error' in k.lower() or 'flag' in k.lower():
                    meta[k] = None
    # ARQUITECTURA DE LIMPIEZA: Entra en modo de observaci├│n (silencio quir├║rgico sin tics)
    try:
        ...
        logging.getLogger(__name__).warning(f'[LIMPIEZA] No se pudo activar modo observaci├│n en StatisticalBrain: {e}')
    logging.getLogger(__name__).warning('[LIMPIEZA] Buffers de error limpiados. Cerebro en modo observaci├│n (silencio quir├║rgico).')
    ...
    # ­ƒø© OMNIPOTENCIA V1.5: Inicializar sistema de descubrimiento universal
    try:
        from core.discovery import OmnipotenceManager
        omnipotence_manager = OmnipotenceManager(system)
        logger.info("­ƒø© OMNIPOTENCIA V1.5 ACTIVADA - Radar Universal en l├¡nea")
    except Exception as omni_err:
        logger.warning(f"ÔÜá´©Å Omnipotencia no disponible: {omni_err}")
        omnipotence_manager = None
        
except Exception as e:
    logger.error(f"Error cr├¡tico al iniciar MeteoSer: {e}", exc_info=True)
    ...
    class Dummy:
        ...
        def obtener_estado_completo(self):
            return {"estado": "Error cr├¡tico en la inicializaci├│n. Ver logs."}
    manager = Dummy()
    system = Dummy()
    omnipotence_manager = None
...
async def iniciar_autodeteccion():
    ...
    # ­ƒø© OMNIPOTENCIA V1.5: Activar radar universal al inicio
    if omnipotence_manager:
        try:
            await omnipotence_manager.start()
            logger.info("­ƒø© Radar Universal iniciado - Buscando hardware por USB/BLE/WiFi...")
        except Exception as omni_err:
    ...
    # ­ƒºá AUTO-GUARDADO DEL CEREBRO ESTAD├ìSTICO
    ...
    if hasattr(system, 'statistical_brain') and system.statistical_brain is not None:
        try:
            ...
            logger.info("­ƒºá Auto-guardado del cerebro activado (cada 100 ciclos)")
    ...
    # ­ƒø© INICIALIZAR OMNIPOTENCIA V1.5 - RADAR UNIVERSAL
    if OMNIPOTENCE_ENABLED and omnipotence:
        try:
            omnipotence.system_core = system
            app.include_router(omnipotence.get_router())
            logger.info("­ƒø© OMNIPOTENCIA V1.5 ACTIVADA - Radar Universal de Hardware")
            logger.info("   Ô£ô Esc├íner USB/Serial")
            logger.info("   Ô£ô Esc├íner Bluetooth BLE")
            logger.info("   Ô£ô Esc├íner WiFi/mDNS")
            logger.info("   Ô£ô Auto-asimilaci├│n de sensores")
            logger.info("   Ô£ô Validaci├│n cruzada de datos")
        except Exception as e:
            logger.warning(f"ÔÜá´©Å Error inicializando Omnipotencia: {e}")
    
    # ­ƒºá Loop de auto-guardado del cerebro
...
async def guardar_cerebro_al_apagar():
    """Guarda el estado del cerebro estad├¡stico antes de apagar el servidor."""
    
    # ­ƒø© Detener Omnipotencia
    if omnipotence_manager:
        try:
            await omnipotence_manager.stop()
            logger.info("­ƒø© Radar Universal detenido")
        except Exception as e:
    ...
    if hasattr(system, 'statistical_brain') and system.statistical_brain is not None:
        try:
            ...
            logger.info("­ƒøæ APAGADO: Guardando estado del cerebro...")
            save_brain_state(system.statistical_brain)
            logger.info("Ô£à Estado del cerebro guardado exitosamente")
        except Exception as e:
            logger.exception(f"ÔØî Error al guardar cerebro durante apagado: {e}")
...
# Endpoint gen├®rico para sensores externos (autoconfigurable)
...
def estado():
    try:
        ...
        logger.error(f"Error en /estado (serializador at├│mico): {e}", exc_info=True)
        return {"error": str(e)}
...
def _estado_impl():
    ...
    if "INDEX_CATALOG" not in globals() or not isinstance(INDEX_CATALOG, dict):
        try:
            ...
            INDEX_CATALOG = {}
    # Refrescar sensores desde el ├║ltimo payload persistido si existe
    ...
    # Aplicar calibraci├│n global si existen factores
    try:
    ...
    # Si no hay sensor de radiaci├│n, estimar radiaci├│n m├íxima posible por arco solar
    if isinstance(persisted_sensores, dict):
    ...
    # A├▒adir arco solar a los ├¡ndices
    ...
    # ├ìndices astron├│micos compuestos
    try:
        ...
        if ventana is not None:
            indices["ventana_observacion_nocturna"] = {
                ...
                "estimado": True,
                "explicacion": "Horas ├║tiles seg├║n cielo observable y duraci├│n de noche"
            }
        ...
        if indice_cielo is not None:
            indices["indice_cielo_astronomico"] = {
                ...
                "estimado": True,
                "explicacion": "├ìndice de cielo astron├│mico (cielo observable + ventana)"
            }
    ...
    bloques_fusionados = {
        ...
        "confort": {
            ...
            "sensacion_termica_ext": {
                ...
                # Humidex eliminado: devolver el mejor ├¡ndice disponible (WBGT > sensacion_calor > vpd)
...
def _resolver_consulta_valor(text: str):
    ...
    if explicacion:
        razon.append(f"Raz├│n: {explicacion}.")
...
async def voz_texto(payload: dict):
    ...
    # --- FEEDBACK POR VOZ ---
    import re
    feedback_match = re.search(r'(acertaste|fue correcta|fue un acierto|fue correcta la|fue buena|fue exacta) (la|el)? ?(predicci[o├│]n|sensor|├¡ndice|indice)? ?(de|del|de la)? ?([\w_\- ]+)', t)
    feedback_error_match = re.search(r'(fallaste|fue incorrecta|fue un error|fue mala|fue err[o├│]nea|fue incorrecto|no acertaste|no fue buena|no fue exacta) (la|el)? ?(predicci[o├│]n|sensor|├¡ndice|indice)? ?(de|del|de la)? ?([\w_\- ]+)', t)
    valor_real_match = re.search(r'(el valor real|el valor correcto|era|fue|deber├¡a ser|deberia ser) ([\d\.,\-]+)', t)
    ...
    if feedback_match or feedback_error_match:
        ...
        valor_estimado = None
        try:
            # Buscar en predicciones, sensores e ├¡ndices
            data = submenu_detallado()
            if nombre:
    ...
    if "submenu" in t or "submen├║" in t:
        data = submenu_detallado()
    ...
    if disponibles.get("total", 0) == 0:
        return _respuesta_voz(session_id, "A├║n no hay datos de sensores recibidos.")
    lista = (disponibles.get("sensores", []) + disponibles.get("derivados", []) + disponibles.get("indices", []))
    muestra = ", ".join(lista[:12])
    return _respuesta_voz(session_id, f"No encontr├® ese valor. Puedes preguntar por: {muestra}.")
...
def _resumen_meteo_actual():
    ...
    if rad is not None:
        detalles.append(f"radiaci├│n {rad} W/m┬▓")
...
# Mantener el endpoint /ecowitt para integraci├│n de sensores
...
async def recibir_ecowitt(request: Request):
    ...
    # SINCRON├ìA TEMPORAL: Inyectar timestamp preciso del dateutc
    try:
        ...
        logging.getLogger(__name__).exception(f"Error en sincron├¡a temporal: {e}")
    ...
    # Conversi├│n y registro de interiores
    ...
    # NOTE: El mapeo y registro de presi├│n se centraliza en
    # `core/integration/ecowitt_receiver.py`. No introducir l├│gica de negocio
    # de sensores aqu├¡ para mantener la puerta de entrada limpia.
    ...
    # Sensores extra: rayos y part├¡culas
    ...
    # Guardar distancia de rayo (si Ecowitt la env├¡a)
    ...
    # Guardar fecha/hora del ├║ltimo rayo
    ...
    # Si existe lightning_num, usarlo como contador absoluto preferente con detecci├│n de reset
    ...
    # --- PM2.5: Registro con ├ímbito interior/exterior ---
    import logging
    ...
    for pm_key in pm_key_candidates:
        try:
            ...
            continue
        # Determinar ├ímbito: si hay tempinf/humidityin -> interior
        ambito = 'exterior'
        if data.get('tempinf') is not None or data.get('humidityin') is not None or data.get('tempin') is not None:
            ambito = 'interior'
        sensor_id = pm_key
        try:
            # REGISTRAR metadata sin par├ímetro ambito (registrar_sensor_metadata no lo acepta)
            system.registrar_sensor_metadata(sensor_id, tipo='pm25', unidad='┬Ág/m┬│', fuente='ecowitt', origen='externo', fiabilidad=90.0)
            ...
            # LOG CR├ìTICO: Confirmar registro de ambito
            logging.getLogger(__name__).warning(f"[ADUANA ECOWITT] Sensor PM {sensor_id} ÔåÆ ambito={ambito} | tempinf={data.get('tempinf')} | humidityin={data.get('humidityin')}")
        except Exception as e:
            logging.getLogger(__name__).error(f"[ADUANA] Error al registrar PM metadata: {e}")
        # Actualizar sensor espec├¡fico
        try:
        ...
        # PUBLICAR sensor gen├®rico 'pm25' con herencia de metadata
        try:
            if hasattr(system, 'sensores_metadata'):
                ...
                logging.getLogger(__name__).warning(f"[HERENCIA AMBITO] pm25 hered├│ ambito={ambito} de {sensor_id}")
        except Exception as herencia_err:
            logging.getLogger(__name__).error(f"[HERENCIA AMBITO] ERROR: {herencia_err}")
        # Actualizar sensor gen├®rico 'pm25'
        try:
    ...
    # Recalcular ├¡ndices tras cada actualizaci├│n (solo c├ílculo, no guardar en system.indices si es un motor)
    ...
    # ­ƒî¬´©Å VALIDACI├ôN CRUZADA CO2/PM2.5 - DETECTOR DE COMBUSTI├ôN
    try:
        ...
        if co2_ppm is not None or pm25_ugm3 is not None:
            ...
            # Log si hay combusti├│n confirmada
            if validation_result["estado"].value == "COMBUSTION_CONFIRMADA":
                logger.warning(f"­ƒöÑ COMBUSTI├ôN CONFIRMADA: {validation_result['recomendacion']}")
    except Exception as e:
        logging.getLogger(__name__).exception(f"Error en validaci├│n cruzada CO2/PM2.5: {e}")
    
    return {"status": "OK", "received": True}


# ­ƒø© ENDPOINT DE OMNIPOTENCIA V1.5
