...
def cargar_duelos_historicos():
    ...
    if not duelos_path.exists():
        print("[ERROR] No hay histórico de duelos")
        return []
...
def verificar_integridad_duelos():
    ...
    if not duelos:
        print("\n[ERROR] No hay duelos en histórico")
        return False
    
    print(f"\n[OK] {len(duelos)} duelos cargados")
    ...
    for i, duelo in enumerate(duelos, 1):
        ...
        if resultado_a is not None and resultado_b is not None:
            if resultado_a > resultado_b:
                print(f"      [OK] CORRECTO - A > B")
                correctos += 1
            else:
                print(f"      [ERROR] ERROR - A <= B (SCORES INVERTIDOS)")
                errores.append({
                    "index": i - 1,
                    "duelo": duelo,
            ...
            print(f"      [WARNING]  No hay scores")
    
    # Resumen
    print(f"\n{'='*70}")
    print("RESUMEN:")
    print(f"{'='*70}")
    print(f"\n[OK] Duelos correctos: {correctos}/{len(duelos)}")
    
    if errores:
        print(f"[ERROR] Duelos con error: {len(errores)}/{len(duelos)}")
        ...
        print(f"\n[OK] TODOS LOS DUELOS SON COHERENTES")
        return True
...
def validar_decisiones():
    ...
    if estadisticas["diferencias"]:
        ...
        if diff_promedio > 0.05:
            print(f"\n[OK] Decisiones tomadas con BUENA MARGEN de separación")
        else:
            print(f"\n[WARNING]  Decisiones muy ajustadas (diferencias pequeñas)")
...
def main():
    ...
    if integridad_ok:
        ...
        print("\n[WARNING]  Corrige los errores de integridad antes de validar decisiones")
