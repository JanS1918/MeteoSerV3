...
def presion_isa_dinamica(altitud_m: float) -> float:
    ...
    Donde:
        ...
        5.255 = exponente (gm / R·L, con g y m del aire seco)
    ...
    Ejemplos:
        ...
        1013.25
        >>> presion_isa_dinamica(118)    # Argentona
        1011.25
    ...
    try:
        ...
        # Validar resultado
        if presion < 0:
            logger.warning(f"[WARNING] Presión ISA negativa a {altitud_m}m: {presion:.2f} hPa")
            return 0.0
        
        return presion
    
    except Exception as e:
        logger.error(f"[ERROR] Error calculando ISA a {altitud_m}m: {e}")
        # Fallback: usar presión media de Argentona (NUNCA 1013.25 genérico)
        from core.system.constants import PRESION
        return PRESION.ARGENTONA_MEDIA  # 1011.3 hPa (118m altitud)
