...
class BusV3Adapter:
    ...
    # Compatibilidad legacy: `leer` usado extensivamente en el código
    def leer(self, variable: str, nivel: str = None, default: Any = None) -> Any:
        """
        Interfaz de compatibilidad para llamadas legacy `bus.leer(...)`.
        - Si se pasa `nivel`, delega en `obtener_valor(variable, nivel=...)`.
        - Si no, delega en `obtener(variable, default=...)`.
        """
        try:
            if nivel is not None:
                return self.obtener_valor(variable, nivel=nivel, default=default)
            return self.obtener(variable, default=default)
        except Exception:
            return default
