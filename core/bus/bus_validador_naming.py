...
class ValidadorNamingColisiones:
    ...
    def registrar(self, nombre: str, modulo: str, version: str = "1.0") -> Tuple[bool, Optional[str]]:
        ...
        with self._lock:
            if nombre in self.registro:
                ...
                msg_colision = (
                    f"[ERROR] COLISIÓN DE NAMING DETECTADA:\n"
                    f"   Nombre: '{nombre}'\n"
                    f"   Intenta: {modulo}\n"
            ...
            logger.debug(f"[OK] Registrado: {nombre} ← {modulo}")
            return True, None
    ...
    def limpiar_ciclo(self):
        ...
        with self._lock:
            ...
            logger.info(f"[REINICIO] Validador de naming reseteado. {len(self.registro)} nombres guardados.")
