...
"""
════════════════════════════════════════════════════════════════════════════════
[REINICIO] INTEGRADOR ALWAYS-ON - AUTO-RECOVER HISTÓRICO EN CADA ARRANQUE
════════════════════════════════════════════════════════════════════════════════
...
class DetectorGapsHistoricos:
    ...
    def detectar_gap(self) -> dict:
        ...
        """
        logger.info("[BUSCAR] Detectando gaps históricos...")
        ...
        if not ultimo_ts:
            logger.warning("[WARNING]  No se encontró último timestamp conocido")
            return {}
        ...
        if gap.total_seconds() > 300:  # > 5 minutos
            logger.warning(f"[WARNING]  GAP DETECTADO: {gap.total_seconds():.0f} segundos ({gap.total_seconds()/3600:.2f}h)")
...
class RecuperadorAutomatico:
    ...
    def recuperar_del_mqtt_local(self) -> list:
        ...
        try:
            ...
            logger.warning(f"[WARNING]  MQTT no disponible: {e}")
    ...
    def recuperar_del_ecowitt_cloud(self) -> list:
        ...
        try:
            ...
            if not all([api_key, api_secret, device_id]):
                logger.info("[WARNING]  Credenciales Ecowitt Cloud no disponibles")
                return []
            ...
            logger.warning(f"[WARNING]  Cloud Ecowitt no disponible: {e}")
    ...
    def recuperar_todo(self, inicio: datetime, fin: datetime) -> dict:
        ...
        logger.info(f"\n[LAUNCH] INICIO RECUPERACIÓN AUTOMÁTICA")
...
class ProcesadorDatosBrutos:
    ...
    def procesar_datos_gap(self, datos: list) -> list:
        ...
        for registro in datos:
            try:
                ...
                logger.warning(f"[WARNING]  Error procesando registro: {e}")
                continue
    ...
    def guardar_en_historico(self, eventos: list) -> Path:
        ...
        logger.info(f"\n[GUARDAR] GUARDANDO EN HISTÓRICO ({len(eventos)} eventos)...")
        ...
        try:
            ...
            logger.error(f"[ERROR] Error guardando histórico: {e}")
            return None
...
async def ejecutar_integrador_automatico():
    ...
    logger.info("\n" + "="*80)
    logger.info("[REINICIO] INTEGRADOR ALWAYS-ON - RECUPERACIÓN AUTOMÁTICA EN ARRANQUE")
    logger.info("="*80 + "\n")
    ...
    try:
        ...
        logger.warning(f"[CRITICAL] GAP DETECTADO: {gap_info['duracion_horas']:.2f} horas")
        ...
        if recuperacion["total_registros"] == 0:
            logger.warning("[WARNING]  No se recuperaron datos del gap")
            return {"status": "gap_sin_datos", "gap_info": gap_info}
        
        logger.info(f"[OK] {recuperacion['total_registros']} registros recuperados")
        ...
        try:
            ...
            logger.warning(f"[WARNING]  Bus no disponible para integración: {e}")
        ...
        logger.error(f"[ERROR] Error en integrador automático: {e}")
        import traceback
...
if __name__ == "__main__":
    ...
    logger.info(f"\n[OK] RESULTADO: {resultado.get('status')}")
