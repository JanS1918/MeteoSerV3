"""
[GUARDIAN] CAPA 31: CENTINELA SOBERANO EXTERNO
Watchdog independiente que vigila el Bus y reinicia el flujo si detecta bloqueo.

CARACTERÍSTICAS CLAVE:
- [OK] Proceso INDEPENDIENTE (no parte de main.py)
- [OK] No puede ser killado por main.py corrupto
- [OK] Reinicia automáticamente si Bus congelado >10s
- [OK] Monitorea heartbeat cada 2s
- [OK] Log independiente (no contamina logs de main)

PROTEGE CONTRA:
[ERROR] Bus congelado (deadlock)
[ERROR] Main.py crasheado pero vivo (zombie)
[ERROR] Loops infinitos silenciosos
[ERROR] Memoria saturada sin detección
...
class CentinelaSoberano:
    ...
    def __init__(self):
        ...
        logger.info("[GUARDIAN] CENTINELA SOBERANO INICIADO")
    ...
    def leer_heartbeat(self) -> Optional[datetime]:
        ...
        try:
            if not self.HEARTBEAT_FILE.exists():
                logger.warning(f"[WARNING] Heartbeat no existe: {self.HEARTBEAT_FILE}")
                return None
            ...
            if not timestamp_str:
                logger.warning("[WARNING] Heartbeat sin timestamp")
                return None
            ...
            logger.error(f"[ERROR] Error leyendo heartbeat: {e}")
            return None
    ...
    def verificar_bloqueo(self) -> bool:
        ...
        if heartbeat is None:
            ...
            if self.ultimo_heartbeat is None:
                logger.warning("[WARNING] Esperando primer heartbeat...")
                return False
            else:
                logger.error("[ERROR] HEARTBEAT DESAPARECIDO")
                return True
        ...
        if tiempo_transcurrido > self.TIMEOUT_BLOQUEO:
            logger.error(
                f"[ERROR] BLOQUEO DETECTADO: {tiempo_transcurrido:.1f}s sin heartbeat "
        ...
        if self.ultimo_heartbeat != heartbeat:
            ...
            logger.debug(f"[OK] Heartbeat OK: {heartbeat.strftime('%H:%M:%S')}")
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
            try:
                ...
                if cmdline and any('main' in arg.lower() for arg in cmdline):
                    logger.info(f"[BUSCAR] Proceso encontrado: PID {proc.pid} - {' '.join(cmdline)}")
                    return proc
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                continue
        
        logger.warning("[WARNING] Proceso main.py no encontrado")
        return None
    ...
    def terminar_proceso_main(self):
        ...
        if proceso is None:
            logger.warning("[WARNING] No hay proceso que terminar")
            return
        
        try:
            logger.warning(f"[WARNING] Terminando proceso PID {proceso.pid}...")
            ...
            try:
                ...
                logger.info("[OK] Proceso terminado suavemente")
                return
            except psutil.TimeoutExpired:
                pass
            
            # Intento 2: Terminación forzada
            logger.warning("[WARNING] Terminación suave falló, forzando...")
            proceso.kill()
            proceso.wait(timeout=3)
            logger.info("[OK] Proceso terminado forzadamente")
            
        except Exception as e:
            logger.error(f"[ERROR] Error terminando proceso: {e}")
    ...
    def reiniciar_sistema(self):
        ...
        logger.warning(
            f"[REINICIO] REINICIANDO SISTEMA (intento {self.reinicios_ejecutados}/{self.MAX_REINTENTOS})"
        )
        ...
        try:
            ...
            logger.info(f"[LAUNCH] Ejecutando: {' '.join(comando)}")
            ...
            logger.info("[OK] Sistema reiniciado")
            
        except Exception as e:
            logger.error(f"[ERROR] Error reiniciando sistema: {e}")
    
    def obtener_estadisticas(self) -> Dict:
        """
        Devuelve estadísticas del Centinela para auditoría del Guardian.
        
        Returns:
            Dict con métricas de vigilancia y salud del sistema
        """
        uptime = (datetime.now() - self.tiempo_inicio).total_seconds()
        
        heartbeat_ok = False
        segundos_sin_heartbeat = 0
        
        if self.ultimo_heartbeat:
            segundos_sin_heartbeat = (datetime.now() - self.ultimo_heartbeat).total_seconds()
            heartbeat_ok = segundos_sin_heartbeat <= self.TIMEOUT_BLOQUEO
        
        return {
            "uptime_segundos": uptime,
            "bloqueos_detectados": self.bloqueos_detectados,
            "reinicios_ejecutados": self.reinicios_ejecutados,
            ...
            "timeout_configurado": self.TIMEOUT_BLOQUEO,
            "proceso_main_activo": self.encontrar_proceso_main() is not None
        }
        ...
        while True:
            try:
                ...
                if hay_bloqueo:
                    ...
                    logger.error(
                        f"[CRITICAL] BLOQUEO #{self.bloqueos_detectados} DETECTADO"
                    )
                    ...
                    if self.reinicios_ejecutados < self.MAX_REINTENTOS:
                        ...
                        logger.critical(
                            f"[CRITICAL] ALERTA CRÍTICA: {self.MAX_REINTENTOS} reintentos fallidos"
                        )
                        logger.critical("[CRITICAL] INTERVENCIÓN MANUAL REQUERIDA")
                ...
                logger.error(f"[ERROR] Error en vigilancia: {e}")
                time.sleep(5)
        
        # Resumen final
        tiempo_total = datetime.now() - self.tiempo_inicio
        logger.info(f"\n[STATS] RESUMEN VIGILANCIA:")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.critical(f"[CRITICAL] FALLO CRÍTICO DEL CENTINELA: {e}")
        sys.exit(1)
