...
class FormulaOptimizationOrchestrator:
    ...
    def ejecutar_ciclo(
        ...
        logger.info(f"\n{'='*60}")
        logger.info(f"[REINICIO] CICLO DE OPTIMIZACIÓN #{self._ciclo_counter}")
        logger.info(f"{'='*60}\n")
        ...
        for parametro in parametros:
            ...
            if not candidatas_validas:
                logger.info(f"   [WARNING]  No hay candidatas válidas para {parametro}")
            ...
            for cand in candidatas_a_duelar:
                ...
                if not fn_interna or not fn_externa:
                    logger.warning(f"   [WARNING]  No se pudo obtener fórmulas para duelo")
                    continue
        ...
        # === Resumen ===
        logger.info(f"\n{'='*60}")
        logger.info(f"[OK] CICLO #{self._ciclo_counter} COMPLETADO")
