...
from core.monitoring.formula_fusion_engine import FormulaFusionEngine

# [OK] Importar recorders para guardar duelos
try:
...
class FormulaDuelEngine:
    def __init__(self, base_dir: Optional[Path] = None):
        ...
        self._optimizer = FormulaOptimizer()  # [OK] Auto-mejora pre-duelo
        self._judge = ThermodynamicJudge()
        self._fusion_engine = FormulaFusionEngine()  # [OK] Evaluación de fusiones
        ...
        # Permitir duelos provisionales cuando hay pocas muestras (mejor que None)
    ...
    def _registrar_pendiente(
        ...
        if (
            ...
            logger.warning(
                "[WARNING] Validación: incoherencias graves para %s (%s). "
        ...
        if pendiente["wins"] >= self.validation_required_wins:
            logger.info(
                "[OK] Validación multi-etapa superada para %s (%s) con %s victorias",
                parametro,
                ganador_id,
                pendiente["wins"],
            )
    ...
    def _duelo_parametro(self, system, parametro: str) -> Optional[Dict[str, Any]]:
        ...
        formula_actual = jerarquia[niveles[0]]  # La mejor actual
        ...
        if not datos or len(datos) < 5:
            if len(datos) < 2:
                logger.warning(f"[CRITICAL] Duelo automático: muestras insuficientes ({len(datos)}) para '{parametro}' - resultado no confiable")
            else:
                logger.info(f"[INFO] Duelo automático: muestras insuficientes ({len(datos)}) para '{parametro}' - resultado provisional")
            # Permitir duelo provisional si hay al menos 1 muestra
            if datos and len(datos) > 0:
                resultado_provisional = {
                    "parametro": parametro,
                    "estado": "provisional",
                    "muestras": len(datos),
                    "razon": "Insuficientes para validación completa"
                }
        ...
        # [OK] Evaluar TODAS las fórmulas de jerarquía
        mejor_formula_jerarquia = formula_actual
        mejor_score_jerarquia = score_actual
        
        for nivel in niveles[1:]:  # Evaluar niveles 1, 2, 3...
            formula = jerarquia[nivel]
            score = self._evaluar_formula(formula, datos, system)
            
            if score and score.score > mejor_score_jerarquia.score:
                # Validar multiround antes de aceptar
                if self._validar_multiround(mejor_formula_jerarquia, formula, datos, system):
                    mejor_formula_jerarquia = formula
                    mejor_score_jerarquia = score
                    logger.info(
                        f"[OK] Fórmula {formula.nombre_tecnico} (nivel {nivel.name}) "
                        f"supera a {mejor_formula_jerarquia.nombre_tecnico}: "
                        f"{score.score:.2f} > {mejor_score_jerarquia.score:.2f}"
                    )

        # [OK] Evaluar TODAS las candidatas (externas + ojeador)
        ...
        mejor_fusion_score = None
        
        for cand in candidatas:
            # [OK] EVALUAR CANDIDATA CON FUSIONES
            # 1. Standalone
            score_cand = self._evaluar_candidata(cand, datos, system)
            if not score_cand:
                continue
            
            # 2. Evaluar como fusión/complemento
            try:
                cand_func = self._resolver_funcion_candidata(cand)
                actual_func = self._resolver_funcion_formula(formula_actual)
                
                if cand_func and actual_func:
                    fusion_score = self._fusion_engine.evaluar_con_fusiones(
                        parametro=parametro,
                        formula_actual=actual_func,
                        formula_actual_id=formula_actual.nombre_tecnico,
                        formula_externa=cand_func,
                        formula_externa_id=cand.get("id", "externa"),
                        datos_prueba=datos
                    )
                    
                    # Si fusión mejora, guardar
                    if fusion_score.mejora_vs_actual > 0:
                        if not mejor_fusion_score or fusion_score.score_total > mejor_fusion_score.score_total:
                            mejor_fusion_score = fusion_score
                            logger.info(
                                f"[OK] Fusión {fusion_score.tipo_fusion} mejora {fusion_score.mejora_vs_actual:.1f}%: "
                                f"{cand.get('id')} + {formula_actual.nombre_tecnico}"
                            )
            
            except Exception as e:
                logger.debug(f"Error evaluando fusión para {cand.get('id')}: {e}")
        ...
        # [OK] Determinar ganador: mejor entre jerarquía, candidatas y fusiones
        ganador = mejor_formula_jerarquia
        mejor_score = mejor_score_jerarquia
        tipo_ganador = "jerarquia"

        # Priorizar robustez y eficiencia si hay empate en score
        def comparar_metricas(a, b):
            if a.score == b.score:
                if hasattr(a, "robustez") and hasattr(b, "robustez"):
                    return a if a.robustez > b.robustez else b
                if hasattr(a, "eficiencia") and hasattr(b, "eficiencia"):
                    return a if a.eficiencia > b.eficiencia else b
            return a if a.score > b.score else b

        # Comparar con mejor candidata standalone
        if mejor_candidata_score:
            mejor_score = comparar_metricas(mejor_score, mejor_candidata_score)
            if mejor_score == mejor_candidata_score:
                ganador = mejor_candidata
                tipo_ganador = "candidata_standalone"

        # Comparar con mejor fusión
        if mejor_fusion_score and mejor_fusion_score.score_total > (mejor_score.score if mejor_score else 0):
            ganador = mejor_fusion_score
            mejor_score = None  # Fusión no tiene DuelScore, tiene FusionScore
            tipo_ganador = f"fusion_{mejor_fusion_score.tipo_fusion}"
            logger.info(
                f"🧬 FUSIÓN GANA: {mejor_fusion_score.tipo_fusion} con score {mejor_fusion_score.score_total:.2f}"
            )

        # Registrar resultados parciales y sugerir mejoras
        if isinstance(mejor_score, DuelScore):
            delta = abs(mejor_score.score - score_actual.score)
        elif isinstance(ganador, dict) and "score_total" in ganador:
            delta = abs(ganador["score_total"] - score_actual.score)
        else:
            delta = 0.0
        ...
        if isinstance(ganador, Fórmula):
            # Ganador de jerarquía
            if tipo_ganador == "jerarquia":
                escenarios = self._escenarios_por_temperatura(system, formula_actual, ganador, datos)
        
        # Guardar resultado según tipo de ganador
        if not self.dry_run:
            if ganador == formula_actual:
                ...
                if isinstance(ganador, Fórmula):
                    ganador_id = ganador.nombre_tecnico
                elif isinstance(ganador, dict):
                    if "score_total" in ganador:
                        # Es FusionScore
                        ganador_id = f"FUSION_{ganador.get('tipo_fusion')}_{ganador.get('formula_principal')}"
                    else:
                        ganador_id = ganador.get("id")
                else:
                    ganador_id = "unknown"
                
                # Calcular delta según tipo de score
                if isinstance(mejor_score, DuelScore):
                    delta = abs(mejor_score.score - score_actual.score)
                elif isinstance(ganador, dict) and "score_total" in ganador:
                    delta = abs(ganador["score_total"] - score_actual.score)
        ...
        # Preparar resultado con información de fusión si aplica
        if isinstance(ganador, Fórmula):
            ganador_nombre = ganador.nombre_legible
            ganador_id_ret = ganador.nombre_tecnico
            score_ret = mejor_score.score if isinstance(mejor_score, DuelScore) else 0
        elif isinstance(ganador, dict):
            if "score_total" in ganador:
                # FusionScore (atributos como dict)
                ganador_nombre = f"Fusión {ganador.get('tipo_fusion')}"
                ganador_id_ret = f"FUSION_{ganador.get('tipo_fusion')}_{ganador.get('formula_principal')}"
                score_ret = ganador.get("score_total", 0)
            else:
                ganador_nombre = ganador.get("nombre", "Externa")
                ganador_id_ret = ganador.get("id")
                score_ret = mejor_score.score if isinstance(mejor_score, DuelScore) else 0
        else:
            ganador_nombre = "Desconocido"
            ganador_id_ret = "unknown"
            score_ret = 0
        
        return {
            "parametro": parametro,
            "ganador": ganador_nombre,
            "ganador_id": ganador_id_ret,
            "ganador_tipo": tipo_ganador,
            "score": score_ret,
            "escenarios": escenarios,
            "fusion_data": ganador if tipo_ganador.startswith("fusion_") else None
        }
    
    def _resolver_funcion_candidata(self, candidata: Dict) -> Optional[Callable]:
        """Resuelve función de candidata externa."""
        try:
            if not candidata:
                return None
            return self._candidate_registry.obtener_funcion(candidata)
        except Exception as e:
            logger.debug(f"Error resolviendo función de candidata: {e}")
            return None

    def auditar_cobertura_total(self, system, incluir_ojeador: bool = True, promover_mejor: bool = False) -> Dict[str, Any]:
        """
        Audita cobertura total de duelos: formulas, candidatas y fusiones.

        Returns:
            Dict con resumen y detalles por parametro.
        """
        resumen = {
            "timestamp": time.time(),
            "parametros_total": len(FORMULA_HIERARCHY),
            "formulas_total": 0,
            "formulas_ok": 0,
        ...
        try:
            for parametro, jerarquia in FORMULA_HIERARCHY.items():
                ...
                niveles = sorted(jerarquia.keys(), key=lambda n: n.value, reverse=True)
                formula_actual = jerarquia[niveles[0]] if niveles else None
                inputs_base = formula_actual.requisitos_datos if formula_actual else []

                candidatas = self._candidate_registry.listar_por_parametro(parametro)
                if incluir_ojeador and formula_actual:
                    candidatas += self._candidatas_de_ojeador(parametro, inputs_base)
                ...
                for formula in formulas:
                    resumen["formulas_total"] += 1
                    samples = self._build_samples(system, formula.requisitos_datos)
                    if not samples:
                        resumen["formulas_sin_datos"] += 1
                        faltan_formulas.append({"id": formula.nombre_tecnico, "razon": "sin_datos"})
                        continue
                if not self._resolver_funcion(formula):
                    resumen["formulas_sin_funcion"] += 1
                    faltan_formulas.append({"id": formula.nombre_tecnico, "razon": "sin_funcion"})
                    continue
                try:
                    score = self._evaluar_formula(formula, samples, system)
                    if score:
                        resumen["formulas_ok"] += 1
                    else:
                        resumen["formulas_error"] += 1
                        faltan_formulas.append({"id": formula.nombre_tecnico, "razon": "sin_score"})
                except Exception:
                    resumen["formulas_error"] += 1
                    faltan_formulas.append({"id": formula.nombre_tecnico, "razon": "error"})

            for cand in candidatas:
                cand_id = cand.get("id") or cand.get("nombre") or "candidata"
                resumen["candidatas_total"] += 1
                requisitos = self._candidate_registry.obtener_inputs(cand)
                samples = self._build_samples(system, requisitos)
                if not samples:
                    resumen["candidatas_sin_datos"] += 1
                    faltan_candidatas.append({"id": cand_id, "razon": "sin_datos"})
                    continue
                if not self._candidate_registry.obtener_funcion(cand):
                    resumen["candidatas_sin_funcion"] += 1
                    faltan_candidatas.append({"id": cand_id, "razon": "sin_funcion"})
                    continue
                try:
                    score = self._evaluar_candidata(cand, samples, system)
                    if score:
                        resumen["candidatas_ok"] += 1
                    else:
                        resumen["candidatas_error"] += 1
                        faltan_candidatas.append({"id": cand_id, "razon": "sin_score"})
                except Exception:
                    resumen["candidatas_error"] += 1
                    faltan_candidatas.append({"id": cand_id, "razon": "error"})

            for formula in formulas:
                formula_fn = self._resolver_funcion(formula)
                for cand in candidatas:
                    cand_id = cand.get("id") or cand.get("nombre") or "candidata"
                    resumen["fusiones_total"] += 1
                    cand_fn = self._candidate_registry.obtener_funcion(cand)
                    if not formula_fn or not cand_fn:
                        resumen["fusiones_sin_funcion"] += 1
                        faltan_fusiones.append({
                            "id": f"{formula.nombre_tecnico}+{cand_id}",
                            "razon": "sin_funcion",
                        })
                        continue
                    requisitos = list({*(formula.requisitos_datos or []), *self._candidate_registry.obtener_inputs(cand)})
                    samples = self._build_samples(system, requisitos)
                    if not samples:
                        resumen["fusiones_sin_datos"] += 1
                        faltan_fusiones.append({
                            "id": f"{formula.nombre_tecnico}+{cand_id}",
                            "razon": "sin_datos",
                        })
                        continue
                    try:
                        self._fusion_engine.evaluar_con_fusiones(
                            parametro=parametro,
                            formula_actual=formula_fn,
                            formula_actual_id=formula.nombre_tecnico,
                            formula_externa=cand_fn,
                            formula_externa_id=cand_id,
                            datos_prueba=samples,
                        )
                        resumen["fusiones_ok"] += 1
                    except Exception:
                        resumen["fusiones_error"] += 1
                        faltan_fusiones.append({
                            "id": f"{formula.nombre_tecnico}+{cand_id}",
                            "razon": "error",
                        })
            ...
            detalles.append({
                ...
                "formulas": {
                    "total": len(formulas),
                    "faltan": faltan_formulas,
                },
                "candidatas": {
                    "total": len(candidatas),
                    "faltan": faltan_candidatas,
                },
                "fusiones": {
                    "total": len(formulas) * len(candidatas),
                    "faltan": faltan_fusiones,
                },
            })
            ...
            logger.exception("[ERROR] auditar_cobertura_total falló: %s", e)
            # Devolver un resumen por defecto en caso de excepción para evitar None
            resumen["detalles"] = detalles
            resumen["error"] = str(e)
            return resumen
    
    def _resolver_funcion_formula(self, formula: Fórmula) -> Optional[Callable]:
        """Resuelve función de fórmula de jerarquía."""
        try:
            # Importar módulo y obtener función
            modulo = importlib.import_module(formula.módulo)
            funcion = getattr(modulo, formula.nombre_tecnico, None)
            return funcion
        except Exception as e:
            logger.debug(f"Error resolviendo función de fórmula: {e}")
            return None
        ...
        # [OK] GUARDAR RESULTADO PARA AUDITORÍA (CRÍTICO PARA APRENDIZAJE)
    ...
    def _validar_multiround(self, formula_actual: Fórmula, formula_alt: Fórmula,
        ...
        if len(datos) < (self.validation_round_min_samples * self.validation_rounds):
            logger.warning(
                "[WARNING] Duelo multiround: datos insuficientes (%s) para %s rondas",
                len(datos),
    ...
    def _guardar_resultado_duelo(self, parametro: str, resultado: Dict[str, Any], datos: List[Dict]):
        """[OK] Guarda el resultado del duelo en histórico para auditoría."""
        ...
        try:
            ...
            # [OK] CORRECCIÓN: Determinar quién es ganador basado en scores
    ...
    def _evaluar_formula(self, formula: Fórmula, datos: List[Dict[str, float]], system) -> Optional[DuelScore]:
        ...
        for sample in datos:
            try:
                ...
                res = self._extract_numeric_value(res)
            ...
            if salida_key:
                ...
                if not juez.ok:
                    for v in juez.violations:
                        logger.warning("[WARNING] Juez termodinámico: %s", v)
                    penalty = self._judge.penalty_factor(juez.violations)
        except Exception:
            penalty = 1.0

        # [OK] APLICAR MEJORAS SIMULADAS (reversible, solo para duelo)
        ...
        if mejoras:
            logger.debug(f"  [OK] {len(mejoras)} mejoras aplicadas a {formula.nombre_tecnico} (+{mejora_pct:.1f}%)")
    ...
    @staticmethod
    def _extract_numeric_value(res) -> Optional[float]:
        if isinstance(res, (int, float)):
            return float(res)
        if isinstance(res, dict):
            if "valor" in res and isinstance(res["valor"], (int, float)):
                return float(res["valor"])
            for key in ("g0_w_m2", "value", "resultado"):
                if key in res and isinstance(res[key], (int, float)):
                    return float(res[key])
            numeric_vals = [v for v in res.values() if isinstance(v, (int, float))]
    ...
    def _build_samples(self, system, requisitos: List[str]) -> List[Dict[str, float]]:
        ...
        if not series:
            if missing_reqs:
                logger.warning(
                    "[WARNING] DuelEngine: sin datos para requisitos %s",
        ...
        if len(muestras) < self.min_clean_samples:
            logger.warning(
                "[WARNING] DuelEngine: muestras insuficientes (%s < %s) para %s",
                len(muestras),
                self.min_clean_samples,
                requisitos,
            )
            # Si permitimos provisional, devolver las muestras disponibles en modo provisional
            if self.allow_provisional and len(muestras) > 0:
                logger.info("[INFO] DuelEngine: usando modo provisional con %s muestras", len(muestras))
                return muestras
            return []
        return muestras
