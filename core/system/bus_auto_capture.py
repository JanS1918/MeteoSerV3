...
class BusAutoCapture:
    ...
    def get_bus(cls):
        ...
        if cls._bus_instance is None:
            logger.warning("[WARNING] Bus no registrado aún en BusAutoCapture")
        ...
        def decorator(func: Callable) -> Callable:
            ...
            def wrapper(*args, **kwargs):
                ...
                if isinstance(result, dict):
                    ...
                    if bus is None:
                        logger.warning(
                            f"[WARNING] Bus no disponible para {prefix}.{func.__name__}"
                        )
                        return result
                    ...
                    for key, value in result.items():
                        ...
                        try:
                            if isinstance(value, dict):
                                ...
                                grifo_inteligente.publicar_si_interes(
                                    bus,
                                    full_name,
                                    value,
                                    unit,
                            ...
                            logger.warning(
                                f"[WARNING] Error publicando {full_name}: {e}"
                            )

                    if published_count > 0:
                        logger.debug(
                            f"[OK] {func.__name__}: {published_count} valores publicados bajo '{prefix}'"
                        )
        ...
        for key, value in data.items():
            ...
            if isinstance(value, dict):
                ...
                try:
                    ...
                    logger.warning(f"[WARNING] Error publicando nested {full_name}: {e}")
            ...
            def wrapper(*args, **kwargs):
                ...
                if result is not None:
                    ...
                    try:
                        ...
                        logger.warning(f"[WARNING] Error publicando {full_name}: {e}")
