...
class BusExpander:
    ...
    async def _delegate_call(self, method_name: str):
        ...
        if method is None:
            logger.error(f"[ERROR] Método no disponible en BusExpander base: {method_name}")
            return
        await method()
    ...
    async def publish_all_subfactors(self):
        ...
        try:
            ...
            logger.info("[OK] BUS V20.0 CERTIFICADO: subfactores reales publicados + gaps cerrados")
        except Exception as e:
            logger.error(f"[ERROR] Error publicando subfactores: {e}", exc_info=True)
    ...
    async def _publish_bus_contract_gaps(self):
        ...
        try:
            ...
            if temp_c is None or humedad is None or presion_pa is None:
                logger.error("[ERROR] No hay datos suficientes para gaps del bus (temperatura/humedad/presión)")
                return
            ...
            try:
                ...
                logger.error(f"[ERROR] Cetrería no disponible: {e}")
        except Exception as e:
            logger.error(f"[ERROR] Error publicando gaps del bus: {e}", exc_info=True)
