...
class PRESION:
    ...
    # PRESIÓN MEDIA ARGENTONA - Reemplaza 1013.25 en fallbacks
    # ISA(118m) = 1011.25 hPa + corrección costera = 1011.3 hPa
    ARGENTONA_MEDIA = 1011.3  # hPa - Fallback específico ubicación
...
# ═══════════════════════════════════════════════════════════════════════════
# [GUARDIAN] TEST DE INTEGRIDAD - SE EJECUTA AL IMPORTAR
# ═══════════════════════════════════════════════════════════════════════════
...
def validar_constitucion():
    ...
    assert ubi['valido'], "[ERROR] ESTACION.LATITUD no es exacta"
    print("  [OK] ESTACION: Coordenadas selladas")
    
    # Test 2: Gravedad
    grav = GRAVEDAD.validar()
    assert grav['valido'], "[ERROR] GRAVEDAD no es exacta"
    assert grav['es_9_81'], "[ERROR] GRAVEDAD se parece a un valor antiguo (INCORRECTO)"
    assert grav['es_isa'], "[ERROR] GRAVEDAD se parece a ISA (INCORRECTO)"
    print("  [OK] GRAVEDAD: 9.80272394 m/s² sellada")
    
    # Test 3: Presión
    pres = PRESION.validar()
    assert pres['usa_formula_barometrica'], "[ERROR] Presión no usa fórmula"
    assert pres['NO_usa_1013_25_hardcoded'], "[ERROR] Presión usa hardcoded"
    print("  [OK] PRESION: Fórmula barométrica unificada")
    
    print("[OK] CONSTITUCION FISICA VALIDADA - SISTEMA EN SOBERANIA\n")
