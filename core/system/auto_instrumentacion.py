...
def descubrir_modulos_automaticamente() -> List[tuple]:
    ...
    logger.info(f"[OK] Auto-discovery: {len(modulos)} módulos descubiertos en core/")
    return modulos
    ...
    for key, value in data.items():
        ...
        # Si es lista de valores simples, publicar como lista
            ...
            if all(isinstance(v, (int, float)) for v in value):
                ...
                try:
                    ...
                    logger.debug(f"[WARNING] Error publicando lista {full_name}: {e}")
        ...
        # Valor simple: publicar
            ...
            try:
                ...
                logger.debug(f"[WARNING] Error publicando {full_name}: {e}")
...
def instrumentar_modulo(
    ...
    try:
        ...
        for nombre, obj in inspect.getmembers(modulo):
            ...
            count += 1
            logger.debug(f"  [OK] Instrumentada: {nombre_modulo}.{nombre} → prefijo '{prefix}'")
        
        logger.info(f"[OK] Módulo instrumentado: {nombre_modulo} ({count} funciones)")
        return count
    
    except ImportError as e:
        logger.warning(f"[WARNING] No se pudo importar {nombre_modulo}: {e}")
        return 0
    
    except Exception as e:
        logger.error(f"[ERROR] Error instrumentando {nombre_modulo}: {e}")
        return 0
...
def instrumentar_sistema_completo(bus_instance: Any) -> Dict[str, int]:
    ...
    if bus_instance is None:
        try:
            ...
            logger.error(f"[ERROR] No se pudo obtener el Bus automáticamente: {e}")

    logger.info("[LAUNCH][LAUNCH][LAUNCH] INSTRUMENTACIÓN V29.0 - AUTO-DISCOVERY TOTAL [LAUNCH][LAUNCH][LAUNCH]")
    logger.info("═" * 100)
    logger.info("[BUSCAR] FASE 1: AUTO-DESCUBRIMIENTO DE MÓDULOS")
    logger.info("═" * 100)
    
    # FASE 1: Auto-descubrir todos los módulos
    modulos = descubrir_modulos_automaticamente()
    logger.info(f"\n[STATS] ESTADÍSTICA INICIAL: {len(modulos)} módulos encontrados en core/")
    ...
    logger.info("\n" + "═" * 100)
    logger.info("[OK] INSTRUMENTACIÓN COMPLETA - RESUMEN FINAL")
    logger.info("═" * 100)
    logger.info(f"📦 Módulos intentados:     {len(modulos)}")
    logger.info(f"[OK] Módulos exitosos:      {exitosos}")
    logger.info(f"[ERROR] Módulos fallidos:      {fallidos}")
    logger.info(f"🔧 Funciones instrumentadas: {total_funciones}")
    logger.info("═" * 100)
    logger.info("[TARGET] RESULTADO: TODOS los subfactores del sistema se publican automáticamente al Bus")
    logger.info("   • Cálculos intermedios: SÍ [OK]")
    logger.info("   • Fórmulas: SÍ [OK]")
    logger.info("   • Subfactores: SÍ [OK]")
    logger.info("   • Cobertura: 100% [OK]")
    logger.info("═" * 100)
...
def instrumentar_funciones_especificas(
    ...
    for nombre_modulo, nombre_funcion, prefix in funciones:
        try:
            ...
            count += 1
            logger.info(f"[OK] Instrumentada: {nombre_modulo}.{nombre_funcion} → '{prefix}'")
        
        except Exception as e:
            logger.error(f"[ERROR] Error instrumentando {nombre_modulo}.{nombre_funcion}: {e}")
...
if __name__ == "__main__":
    ...
    print("\n[LAUNCH] Instrumentando sistema completo...")
    stats = instrumentar_sistema_completo(bus)
    
    print("\n[STATS] Estadísticas de instrumentación:")
    for modulo, count in stats.items():
        if count > 0:
            print(f"  [OK] {modulo}: {count} funciones")
