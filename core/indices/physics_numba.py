"""
PHYSICS ENGINE PURO - Sin dependencias externas
===============================================================
Motor de cálculos físicos en Python puro (sin numpy/numba/scipy).
Pureza local: solo math stdlib + core internals.

Uso:
    from core.indices.physics_numba import PhysicsEnginePuro
    
    engine = PhysicsEnginePuro()
...
def jit(*args, **kwargs):
    """Fallback para numba.jit cuando numba no está disponible."""
    if _jit is None:
        def _decorador(func):
            return func
        return _decorador
...
# CONSTANTES FÍSICAS
# ============================================================================
# CRÍTICO: G ahora viene de core.system.constants.GRAVEDAD
# Unificado a la gravedad sellada de Argentona (sin valores antiguos)
from core.system.constants import GRAVEDAD
G = GRAVEDAD.DINAMICA  # Gravedad - UNIFICADO desde Bus
...
# ============================================================================
# FUNCIONES PURAS (Sin numpy - math stdlib only)
# ============================================================================

def presion_vapor_saturacion_puro(T_celsius: float) -> float:
    """Presión vapor saturación (Magnus-Tetens) [hPa]"""
    return 6.1094 * math.exp(17.625 * T_celsius / (T_celsius + 243.04))


def presion_vapor_actual_puro(T_celsius: float, HR_pct: float) -> float:
    """Presión vapor actual [hPa]"""
    es = presion_vapor_saturacion_puro(T_celsius)
    return (HR_pct / 100.0) * es


def densidad_aire_puro(T_celsius: float, P_hpa: float, HR_pct: float) -> float:
    """Densidad del aire [kg/m³]"""
    T_kelvin = T_celsius + 273.15
    e = presion_vapor_actual_puro(T_celsius, HR_pct)
    rho = (P_hpa * 100.0 - 0.3783 * e * 100.0) / (R_SPECIFIC_DRY * T_kelvin)
    return rho


def viscosidad_aire_puro(T_celsius: float) -> float:
...
def velocidad_sonido_numba(T_celsius: float) -> float:
    ...
    gamma = 1.4
    if np is None:
        return math.sqrt(gamma * R_SPECIFIC_DRY * T_kelvin)
...
def escala_logaritmica_viento_numba(z: float, z0: float, u_star: float) -> float:
    ...
    if z <= z0 or u_star == 0:
        return 0.0
    if np is None:
        return (u_star / KARMAN) * math.log(z / z0)
...
def temperatura_punto_rocio_numba(T_celsius: float, HR_pct: float) -> float:
    ...
    b = 237.7
    if np is None:
        alpha = (a * T_celsius) / (b + T_celsius) + math.log(HR_pct / 100.0)
    else:
...
class PhysicsEngineNumba:
    ...
    def __init__(self):
        if not NUMBA_AVAILABLE:
            print("[WARNING]  Numba no disponible, funcionará en modo Python puro (más lento)")
...
if __name__ == "__main__":
    ...
    if NUMBA_AVAILABLE:
        ...
        print("\n[STATS] BENCHMARK: PhysicsEngineNumba")
        print("   " + "-"*60)
        ...
        print(f"\n   [OK] Aceleración: {tiempo_scalar/tiempo_batch:.1f}x más rápido")
    else:
        print("\n[ERROR] Numba no disponible. Instalarlo con: pip install numba")
