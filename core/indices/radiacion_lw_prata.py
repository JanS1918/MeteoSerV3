...
def calcular_lw_prata_1996(T_air_c: float, RH_pct: float, P_hpa: float = 1013.25) -> dict:
    """
    Compatibilidad legacy: interfaz usada por wrappers antiguos.
    Convierte entradas y devuelve un dict con claves esperadas.
    """
    try:
        from core.indices.environmental_indices import saturacion_vapor_hyland_wexler
    except Exception:
        saturacion_vapor_hyland_wexler = None

    presion_pa = float(P_hpa) * 100.0
    # Temperatura en Kelvin
    T_k = float(T_air_c) + 273.15

    # Calcular presión de vapor real (fallback simple)
    if saturacion_vapor_hyland_wexler is not None:
        try:
            es_pa = float(saturacion_vapor_hyland_wexler(float(T_air_c), presion_pa))
        except Exception:
            es_pa = None
    else:
        es_pa = None
    ...
    if es_pa is None:
        # Magnus-Tetens fallback en hPa → Pa
        try:
            import math
            a = 6.116441
            b = 17.62391
            c = 243.12
            ln_es = (b * float(T_air_c)) / (c + float(T_air_c))
            es_hpa = a * math.exp(ln_es)
            es_pa = es_hpa * 100.0
        except Exception:
            es_pa = 0.0

    e_pa = es_pa * (float(RH_pct) / 100.0)

    lw = calcular_radiacion_lw_descendente_prata(T_k, e_pa)
    eps = calcular_emissividad_cielo_prata(T_k, e_pa)
    t_sky = calcular_temperatura_cielo_efectiva_prata(T_k, e_pa)

    return {
        'LW_cielo_wm2': lw,
        'epsilon_cielo': eps,
        'T_cielo_efectiva_k': t_sky,
        'es_pa': es_pa,
        'e_pa': e_pa
    }
