...
def generar_alertas_dinamicas(
    ...
    # 3b. ALERTA LLUVIA INTENSA (usa lluvia_rate si existe)
    if precipitacion_rate_mm_h >= 20:
        alertas["alerta_lluvia_intensa"] = {
            "nivel": "rojo",
            "valor": precipitacion_rate_mm_h,
            "razon": f"Lluvia {precipitacion_rate_mm_h:.1f} mm/h >= 20 mm/h (muy intensa)",
            "recomendacion": "Evitar desplazamientos, riesgo de anegamientos"
        }
    elif precipitacion_rate_mm_h >= 10:
        alertas["alerta_lluvia_intensa"] = {
            "nivel": "naranja",
            "valor": precipitacion_rate_mm_h,
            "razon": f"Lluvia {precipitacion_rate_mm_h:.1f} mm/h >= 10 mm/h (intensa)",
            "recomendacion": "Precaución en exteriores y conducción"
        }
    elif precipitacion_rate_mm_h >= 5:
        alertas["alerta_lluvia_intensa"] = {
            "nivel": "amarilla",
            "valor": precipitacion_rate_mm_h,
            "razon": f"Lluvia {precipitacion_rate_mm_h:.1f} mm/h >= 5 mm/h (moderada)",
            "recomendacion": "Llevar protección y revisar drenajes"
        }
