...
def calcular_radiacion_extraterrestre_rest2(
    ...
    elevacion_spa_nrel: float = None,
    azimut_spa_nrel: float = None
) -> Dict[str, float]:
    """
    Radiación Solar Extraterrestre (G₀) usando REST2 (Gueymard 2008).
    
    ⚛️ V42.6 ANCLAJE TOTAL SPA NREL:
    Si elevacion_spa_nrel y azimut_spa_nrel se proporcionan, REST2 USA EXCLUSIVAMENTE
    esos valores de posición solar en lugar de calcular su propia astronomía.
    
    Esto garantiza que cada vatio de radiación calculado por Gueymard se base en la
    posición real del sol en Argentona con precisión de NREL SPA (segundos de arco).
    ...
    FLUJO COMPUTACIONAL V42.6:
    1. Factor de excentricidad orbital
    2. [OPCIONAL] Tiempo solar verdadero (solo si no hay SPA)
    3. [OPCIONAL] Declinación solar (solo si no hay SPA)
    4. [OPCIONAL] Ángulo horario (solo si no hay SPA)
    5. Ángulo zenital → Elevación → Masa de aire (SPA o interno)
    ...
    Args:
        ...
        elevacion_spa_nrel: [V42.6] Elevación solar aparente desde SPA NREL (°)
        azimut_spa_nrel: [V42.6] Azimuth solar desde SPA NREL (°)
    ...
    Returns:
        Diccionario con:
          ...
          - fuente_astronomica: "SPA_NREL" o "REST2_interno"
    """
    
    # 1. Factor de excentricidad (siempre se calcula)
    f_exc = calcular_factor_excentricidad_orbital(fecha)
    
    # 2-5. ANCLAJE V42.6: Usar SPA NREL si está disponible
    if elevacion_spa_nrel is not None:
        # ⚛️ MODO ANCLADO: Usar posición SPA NREL directamente
        h_deg = elevacion_spa_nrel
        fuente_astronomica = "SPA_NREL_V42.6"
    else:
        # Modo legacy: calcular astronomía interna REST2
    ...
    return {
        ...
        "fuente_astronomica": fuente_astronomica,  # V42.6: Identifica origen de posición solar
    }
...
if __name__ == "__main__":
    ...
    print("\n" + "═" * 80)
    print("[OK] REST2 (Gueymard) operacional. Radiación extraterrestre confirmada.")
    print("═" * 80)
