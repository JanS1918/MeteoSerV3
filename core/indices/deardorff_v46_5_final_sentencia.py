...
def generar_hash_adn_geografia():
    ...
    datos_adn = {
        ...
        "coordenadas": f"{bus.obtener_valor('contexto.ubicacion.latitud')}, {bus.obtener_valor('contexto.ubicacion.longitud')}",
        "altitud_m": bus.obtener_valor('contexto.ubicacion.altitud_sensor'),
        "horizonte_topografico_grados": bus.obtener_valor('contexto.terreno.horizonte_promedio_grados'),
        "bosques_cercanos": bus.obtener_valor('contexto.terreno.bosque_cercano'),
        "tipo_suelo_primario": bus.obtener_valor('contexto.terreno.tipo_suelo_primario'),
        "conductividad_k_s_wm_k": bus.obtener_valor('contexto.terreno.conductividad_suelo_wm2k'),
        "factor_inercia_edificio": bus.obtener_valor('contexto.terreno.factor_retorno_termico_pared_wm2'),
...
def calcular_temperatura_minima_v46_5_final(
    ...
    # Obtener parámetros de suelo desde el bus
    bus = obtener_bus()
    params = {
        "C_s": bus.obtener_valor('contexto.terreno.capacidad_calorica_suelo'),
        "k_s": bus.obtener_valor('contexto.terreno.conductividad_suelo_wm2k'),
        "z_d": bus.obtener_valor('contexto.terreno.profundidad_efectiva_m'),
        "nombre": bus.obtener_valor('contexto.terreno.tipo_suelo_primario')
    }
    C_s = params["C_s"]
    ...
    inercia = InerciaTermicaEdificio(
        factor_wm2=bus.obtener_valor('contexto.terreno.factor_retorno_termico_pared_wm2'),
        duracion_h=bus.obtener_valor('contexto.terreno.duracion_inercia_h')
    )
    ...
    for step in range(n_steps):
        ...
        if bus.obtener_valor('contexto.terreno.ocaso_topografico_bloquea_evaporacion') and h_ocaso > 0:
    ...
    if aplicar_correccion_bosque and bus.obtener_valor('contexto.terreno.bosque_cercano'):
        corrector = CorreccionRadiacionBosque(
            distancia_bosque_m=bus.obtener_valor('contexto.terreno.distancia_bosque_min_m'),
            tipo_bosque=bus.obtener_valor('contexto.terreno.tipo_bosque')
        )
    ...
    if np.ndim(temperatura_actual_c) == 0:
        return {
            ...
            "duracion_inercia_edificio_h": bus.obtener_valor('contexto.terreno.duracion_inercia_h'),
            "ocaso_bloquea_evaporacion": bus.obtener_valor('contexto.terreno.ocaso_topografico_bloquea_evaporacion'),
