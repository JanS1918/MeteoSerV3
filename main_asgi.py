...
def _imprimir_item(item: dict) -> dict:
    try:
        ...
        item["status"] = status
        item["printed_at"] = datetime.datetime.utcnow().isoformat() if printed else None
    except Exception:
        item["status"] = "error"
    return item
...
def _estado_impl():
    ...
    try:
        ...
        if cielo_val is not None and ventana is not None:
            cielo_n = max(0.0, min(1.0, float(cielo_val) / 100.0))
            horas_n = max(0.0, min(1.0, float(ventana) / 6.0))
            indice_cielo = (0.7 * cielo_n + 0.3 * horas_n) * 100.0
...
def submenu_detallado():
    try:
        ...
        return JSONResponse(content=_json_safe(fallback))
...
def _ruta_layout() -> pathlib.Path:
    path = BASE_DIR / "data" / "dashboard_layout.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    return path


def _ruta_paneles() -> pathlib.Path:
    path = BASE_DIR / "data" / "dashboard_panels.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    return path
